...
from atulya_api.models import Base


# Load environment variables based on ATULYA_API_DATABASE_URL env var or default to local
def load_env():
    """Load environment variables from .env"""
    # Check if ATULYA_API_DATABASE_URL is already set (e.g., by CI/CD)
    if os.getenv("ATULYA_API_DATABASE_URL"):
        return
    ...
    if not database_url:
        ...
        if not database_url:
            raise ValueError(
                ...
                "Set ATULYA_API_DATABASE_URL environment variable or pass database_url to run_migrations()."
            )
