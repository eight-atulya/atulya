...
Configuration via environment variables - see atulya_api.config for all env var names.
"""
...
class LocalSTCrossEncoder(CrossEncoderModel):
    ...
    async def initialize(self) -> None:
        ...
        if self.force_cpu:
            device = "cpu"
            logger.info("Reranker: forcing CPU mode (ATULYA_API_RERANKER_LOCAL_FORCE_CPU=1)")
        else:
