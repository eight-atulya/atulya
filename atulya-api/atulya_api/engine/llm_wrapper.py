...
# Set ATULYA_API_LLM_MAX_CONCURRENT=1 for local LLMs (LM Studio, Ollama)
...
def sanitize_llm_output(text: str | None) -> str | None:
    ...
    Removes:
    - ASCII control characters (0x00-0x08, 0x0B-0x0C, 0x0E-0x1F, 0x7F): these
      can break JSON parsing and PostgreSQL UTF-8 handling. Tab (0x09), newline
      (0x0A), and carriage return (0x0D) are preserved.
    - Unicode surrogates (U+D800-U+DFFF): invalid in UTF-8.
    """
    if text is None:
        return None
    if not text:
...
class LLMProvider:
    ...
    def __init__(
        ...
        if self.provider == "vertexai":
            ...
            if not vertexai_project_id:
                raise ValueError(
                    "ATULYA_API_LLM_VERTEXAI_PROJECT_ID is required for Vertex AI provider. "
    ...
    def with_config(self, config: Any) -> "ConfiguredLLMProvider":
        ...
        Args:
            config: Resolved ``AtulyaConfig`` for the current bank/request.
    ...
    def for_memory(cls) -> "LLMProvider":
        ...
        provider = os.getenv("ATULYA_API_LLM_PROVIDER", "groq")
        api_key = os.getenv("ATULYA_API_LLM_API_KEY", "")
        ...
        if not api_key and provider not in ("openai-codex", "claude-code", "ollama", "vertexai"):
            raise ValueError(
                "ATULYA_API_LLM_API_KEY environment variable is required (unless using openai-codex or claude-code)"
            )

        base_url = os.getenv("ATULYA_API_LLM_BASE_URL", "")
        model = os.getenv("ATULYA_API_LLM_MODEL", "openai/gpt-oss-120b")
    ...
    def for_answer_generation(cls) -> "LLMProvider":
        ...
        provider = os.getenv("ATULYA_API_ANSWER_LLM_PROVIDER", os.getenv("ATULYA_API_LLM_PROVIDER", "groq"))
        api_key = os.getenv("ATULYA_API_ANSWER_LLM_API_KEY", os.getenv("ATULYA_API_LLM_API_KEY", ""))

        # API key not needed for openai-codex (uses OAuth), claude-code (uses Keychain OAuth),
        # ollama (local), or vertexai (uses GCP service account credentials)
        if not api_key and provider not in ("openai-codex", "claude-code", "ollama", "vertexai"):
            raise ValueError(
                "ATULYA_API_LLM_API_KEY or ATULYA_API_ANSWER_LLM_API_KEY environment variable is required "
                "(unless using openai-codex or claude-code)"
            )

        base_url = os.getenv("ATULYA_API_ANSWER_LLM_BASE_URL", os.getenv("ATULYA_API_LLM_BASE_URL", ""))
        model = os.getenv("ATULYA_API_ANSWER_LLM_MODEL", os.getenv("ATULYA_API_LLM_MODEL", "openai/gpt-oss-120b"))
    ...
    def for_judge(cls) -> "LLMProvider":
        ...
        provider = os.getenv("ATULYA_API_JUDGE_LLM_PROVIDER", os.getenv("ATULYA_API_LLM_PROVIDER", "groq"))
        api_key = os.getenv("ATULYA_API_JUDGE_LLM_API_KEY", os.getenv("ATULYA_API_LLM_API_KEY", ""))

        # API key not needed for openai-codex (uses OAuth), claude-code (uses Keychain OAuth),
        # ollama (local), or vertexai (uses GCP service account credentials)
        if not api_key and provider not in ("openai-codex", "claude-code", "ollama", "vertexai"):
            raise ValueError(
                "ATULYA_API_LLM_API_KEY or ATULYA_API_JUDGE_LLM_API_KEY environment variable is required "
                "(unless using openai-codex or claude-code)"
            )

        base_url = os.getenv("ATULYA_API_JUDGE_LLM_BASE_URL", os.getenv("ATULYA_API_LLM_BASE_URL", ""))
        model = os.getenv("ATULYA_API_JUDGE_LLM_MODEL", os.getenv("ATULYA_API_LLM_MODEL", "openai/gpt-oss-120b"))
