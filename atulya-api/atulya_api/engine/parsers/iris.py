"""Iris parser implementation using the eightengine Iris HTTP API."""
...
_IRIS_BASE_URL = "https://api.eightengine.com/v1"
...
class IrisParser(FileParser):
    """
    Iris file parser using the eightengine Iris cloud extraction service.

    Uploads files to the eightengine Iris API, starts an extraction job,
    ...
    Authentication:
        Requires ATULYA_API_FILE_PARSER_IRIS_TOKEN and
        ATULYA_API_FILE_PARSER_IRIS_ORG_ID environment variables,
    ...
    def __init__(
        ...
        Args:
            token: eightengine API token
            org_id: eightengine organization ID
    ...
    async def convert(self, file_data: bytes, filename: str) -> str:
        """
        Parse file to text using the eightengine Iris API.
        ...
        try:
            ...
            raise RuntimeError(f"Iris request timed out for '{filename}': {e}") from e
        except httpx.RequestError as e:
            raise RuntimeError(f"Iris request failed for '{filename}': {e}") from e
