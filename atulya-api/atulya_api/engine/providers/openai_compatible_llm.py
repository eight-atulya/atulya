...
from atulya_api.config import DEFAULT_LLM_TIMEOUT, ENV_LLM_TIMEOUT
from atulya_api.engine.llm_interface import LLMInterface, OutputTooLongError
from atulya_api.engine.response_models import LLMToolCall, LLMToolCallResult, TokenUsage
from atulya_api.metrics import get_metrics_collector
...
class OpenAICompatibleLLM(LLMInterface):
    ...
    def _parse_json_with_fallback(self, content: str | None) -> dict[str, Any]:
        """
        Parse JSON robustly across providers that may wrap payloads in prose/code fences.
        """
        ...
        # JSON object substring fallback (handles "Here is JSON: {...}")
        start = raw.find("{")
        end = raw.rfind("}")
        if 0 <= start < end:
        ...
        if isinstance(last_err, json.JSONDecodeError):
            raise last_err
        raise json.JSONDecodeError("invalid json content", raw, 0)
        ...
        for attempt in range(max_retries + 1):
            try:
                if response_format is not None:
                    ...
                    try:
                        json_data = self._parse_json_with_fallback(content)
                        ...
                        if attempt < max_retries:
                            ...
                            continue
                        # Deterministic fallback when validation is optional
                        if skip_validation:
                            logger.warning("Structured parse failed; returning deterministic empty object fallback")
                            json_data = {}
                        else:
                ...
                # Record trace span
                from atulya_api.tracing import _serialize_for_span, get_span_recorder
                ...
                if e.status_code in (401, 403):
                    logger.error(
                        f"Auth error (HTTP {e.status_code}, {self.provider}/{self.model}), "
    ...
    async def call_with_tools(
        ...
        effective_messages, effective_tools, effective_tool_choice = self._normalize_tool_choice_for_provider(
            messages=messages,
            tools=tools,
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                from atulya_api.tracing import get_span_recorder
    ...
    def _normalize_tool_choice_for_provider(
        self,
        messages: list[dict[str, Any]],
        tools: list[dict[str, Any]],
        ...
        """
        Normalize tool_choice for provider compatibility.

        Some providers (e.g. LM Studio/Ollama OpenAI-compatible endpoints) reject
        OpenAI named-function tool_choice objects and only accept string values.
        For those providers, we preserve deterministic behavior by:
        1) converting tool_choice to "required"
        2) constraining available tools to the requested function when possible
        """
        supports_named_tool_choice = self.provider in ("openai", "groq")

        if not isinstance(tool_choice, dict) or supports_named_tool_choice:
        ...
        if tool_choice.get("type") == "function":
            ...
            if isinstance(function_obj, dict):
                requested_function = str(function_obj.get("name") or "")
        ...
        if requested_function:
            constrained_tools = [
                ...
                and tool["function"].get("name") == requested_function
            ]
            if constrained_tools:
                logger.info(
                    "Provider %s does not support named tool_choice; using tool_choice='required' "
                    "with constrained tool set: %s",
                    self.provider,
                    requested_function,
                )
                return messages, constrained_tools, "required"
            logger.warning(
                "Provider %s does not support named tool_choice and requested tool '%s' "
                "was not found in tools; falling back to tool_choice='required' with full tool list",
                self.provider,
                requested_function,
            )
        else:
            logger.info(
                "Provider %s does not support object tool_choice; falling back to tool_choice='required'",
                self.provider,
            )

        return messages, tools, "required"
                ...
                try:
                    ...
                    try:
                        ...
                        if attempt < max_retries:
                            ...
                            if skip_validation:
                                logger.warning("Ollama parse failed; returning deterministic empty object fallback")
                                json_data = {}
                            else:
                                raise
