...
class ClaudeCodeLLM(LLMInterface):
    ...
    async def call_with_tools(
        ...
        format to SDK MCP tools, and tool names are formatted as mcp__atulya_tools__{name}.
        ...
        allowed_tool_names = [f"mcp__atulya_tools__{name}" for name in tool_names]

        # Configure SDK options with MCP server
        options = ClaudeAgentOptions(
            system_prompt=system_prompt if system_prompt else None,
            max_turns=1,  # Single-turn for API-style interactions
            mcp_servers={"atulya_tools": mcp_server} if sdk_tools else {},
                        ...
                        if isinstance(message, AssistantMessage):
                            for block in message.content:
                                if isinstance(block, TextBlock):
                                    ...
                                    # SDK returns tool names with MCP prefix (mcp__atulya_tools__{name})
                                    # Strip the prefix to return original tool name expected by caller
                                    tool_name = block.name
                                    if tool_name.startswith("mcp__atulya_tools__"):
                                        tool_name = tool_name.replace("mcp__atulya_tools__", "", 1)
