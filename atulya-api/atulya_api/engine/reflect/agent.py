...
# Some local / chat-tuned models emit special tokens instead of clean assistant text, e.g.:
#   <|channel|>final <|constrain|>commentary to=functions.done<|constrain|>json<|message|>{"answer":"..."}
_HF_STYLE_TOKEN = re.compile(r"<\|[^|]+\|>")
...
def _parse_json_answer_field(fragment: str) -> str | None:
    """Parse the first JSON object in ``fragment`` and return ``answer`` if it is a non-empty string."""
...
def _extract_reflect_answer_from_model_output(text: str) -> str | None:
    """
    Extract the user-visible answer when the model emits channel tokens + JSON instead of plain text.

    Common with some local OpenAI-compatible models (LM Studio, etc.) that emulate tools by printing
    structures like ``<|message|>{"answer":"..."}`` rather than returning native ``tool_calls``.

    Returns ``None`` if no extractable ``answer`` field is found (caller keeps normal cleanup path).
    """
    if not text or not text.strip():
        return None
    s = text.strip()

    # 1) Explicit <|message|> ... JSON (most reliable for this family of outputs)
    if "<|message|>" in s:
        tail = s.split("<|message|>", 1)[1].strip()
        parsed = _parse_json_answer_field(tail)
        if parsed is not None:
            return parsed

    # 2) Entire body is a single JSON object with "answer"
    if s.startswith("{"):
        parsed = _parse_json_answer_field(s)
        if parsed is not None:
            return parsed

    # 3) Strip <|...|> tokens, then find first { ... } JSON with "answer"
    stripped = _HF_STYLE_TOKEN.sub("", s).strip()
    if stripped != s:
        i = stripped.find("{")
        if i >= 0:
            parsed = _parse_json_answer_field(stripped[i:])
            if parsed is not None:
                return parsed
...
def _clean_answer_text(text: str) -> str:
    ...
    Also extracts ``answer`` from channel-token + JSON blobs emitted by some local models.
    """
    extracted = _extract_reflect_answer_from_model_output(text)
...
def _clean_done_answer(text: str) -> str:
    ...
    - Channel tokens + JSON (same as _clean_answer_text) when the model puts the full blob in ``answer``
...
async def run_reflect_agent(
    ...
    for iteration in range(max_iterations):
        ...
        if done_call:
            ...
            from atulya_api.tracing import get_tracer

            tracer = get_tracer()
            span_name = "atulya.reflect_tool_call"
            with tracer.start_as_current_span(span_name) as span:
                span.set_attribute("atulya.scope", "reflect_tool_call")
                span.set_attribute("atulya.operation", "reflect_tool_call")
...
async def _execute_tool_with_timing(
    ...
    span_name = f"atulya.reflect_tool_exec.{normalized_name}"
    ...
    with tracer.start_as_current_span(
        ...
        # Set attributes
        span.set_attribute("atulya.tool.name", normalized_name)
        span.set_attribute("atulya.tool.id", tc.id)
        span.set_attribute("atulya.tool.arguments", json.dumps(tc.arguments))
