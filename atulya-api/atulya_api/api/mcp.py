...
from atulya_api.engine.memory_engine import _current_schema
from atulya_api.extensions import MCPExtension, load_extension
...
# Configure logging from ATULYA_API_LOG_LEVEL environment variable
_log_level_str = os.environ.get("ATULYA_API_LOG_LEVEL", "info").lower()
_log_level_map = {
...
DEFAULT_BANK_ID = os.environ.get("ATULYA_MCP_BANK_ID", "default")
...
MCP_AUTH_TOKEN = os.environ.get("ATULYA_API_MCP_AUTH_TOKEN")
...
def create_mcp_server(memory: MemoryEngine, multi_bank: bool = True) -> FastMCP:
    """
    Create and configure the Atulya MCP server.
...
class MCPMiddleware:
    ...
    Authentication:
        1. If ATULYA_API_MCP_AUTH_TOKEN is set (legacy), validates against that token
    ...
    1. Multi-bank mode (for /mcp/ root endpoint):
       ...
       - Bank ID from: X-Bank-Id header or ATULYA_MCP_BANK_ID env var
    ...
    Bank ID resolution priority:
        ...
        3. ATULYA_MCP_BANK_ID env var → multi-bank mode (default: "default")
