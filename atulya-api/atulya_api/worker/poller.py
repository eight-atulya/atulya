...
class WorkerPoller:
    ...
    def __init__(
        ...
        Args:
            ...
            sub_routine_max_slots: Maximum concurrent sub_routine tasks per worker
        """
        self._pool = pool
    ...
    async def _get_available_slots(self) -> tuple[int, int, int]:
        """
            ...
            async with conn.transaction():
                ...
                if sub_routine_limit > 0:
                    sub_routine_rows = await conn.fetch(
                        ...
                        WHERE status = 'pending'
                          AND task_payload IS NOT NULL
                          AND operation_type IN ('sub_routine', 'brain_learn')
                          AND (next_retry_at IS NULL OR next_retry_at <= NOW())
                          AND NOT EXISTS (
                              SELECT 1 FROM {table} AS processing
                              WHERE processing.bank_id = pending.bank_id
                                AND processing.operation_type IN ('sub_routine', 'brain_learn')
                                AND processing.status = 'processing'
                          )
                        ORDER BY created_at
                        LIMIT $1
                ...
                for row in all_rows:
                    ...
                    if "operation_type" not in task_dict:
                        task_dict["operation_type"] = row["operation_type"]
    ...
    async def run(self):
        ...
        logger.info(
            ...
            f"sub_routine_max_slots={self._sub_routine_max_slots})"
        )

        while not self._shutdown.is_set():
            try:
                await self._enqueue_due_dream_cron_tasks()
                ...
                if tasks:
                    ...
                    logger.info(
                        ...
                        f"({consolidation_count} consolidation, {sub_routine_count} sub_routine): "
                        f"{types_str} (schemas: {schemas_str})"
                    )
    ...
    async def _enqueue_due_dream_cron_tasks(self) -> None:
        """Best-effort cron scheduler for Dream/Trance mode (hybrid/cron trigger modes)."""
        now = time.time()
        if now - self._last_dream_cron_scan < 60:
            return
        self._last_dream_cron_scan = now
        schemas = await self._get_schemas()
        for schema in schemas:
            try:
                await self._enqueue_due_dream_cron_tasks_for_schema(schema)
            except Exception as e:
                logger.debug("dream cron scan failed for schema %s: %s", schema or "default", e)
        ...
        async with self._pool.acquire() as conn:
            ...
            for row in rows:
                ...
                if not isinstance(dream, dict):
                    continue
                enabled = bool(dream.get("enabled"))
                trance = bool(dream.get("trance_enabled"))
                trigger_mode = str(dream.get("trigger_mode", "hybrid")).lower()
                if not enabled or not trance or trigger_mode not in ("cron", "hybrid"):
                    continue
                interval_minutes = max(5, min(int(dream.get("cron_interval_minutes", 180)), 24 * 60))
                pending = await conn.fetchrow(
                    f"""
                    SELECT operation_id
                    FROM {ops_table}
                    WHERE bank_id = $1 AND operation_type = 'dream_generation' AND status IN ('pending', 'processing')
                    LIMIT 1
                    """,
                    bank_id,
                )
                ...
                latest = await conn.fetchrow(
                    ...
                    SELECT created_at
                    FROM {dream_table}
                    WHERE bank_id = $1 AND trigger_source = 'cron'
                ...
                if latest and latest["created_at"]:
                    age_seconds = time.time() - latest["created_at"].timestamp()
                ...
                await conn.execute(
                    f"""
                    INSERT INTO {ops_table} (operation_id, bank_id, operation_type, result_metadata, status, task_payload)
                    VALUES ($1, $2, 'dream_generation', $3::jsonb, 'pending', $4::jsonb)
                    """,
                    operation_id,
                    bank_id,
                    json.dumps({"trigger_source": "cron", "run_type": "trance", "operation_stage": "queued"}),
                    json.dumps(payload),
                )
