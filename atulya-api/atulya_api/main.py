...
from .config import DEFAULT_WORKERS, ENV_WORKERS, AtulyaConfig, _get_raw_config
from .daemon import (
    DEFAULT_DAEMON_PORT,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="atulya-api",
        description="Atulya API Server",
    )

    # Server options
    parser.add_argument(
        "--host", default=config.host, help=f"Host to bind to (default: {config.host}, env: ATULYA_API_HOST)"
    )
    parser.add_argument(
        "--port",
        type=int,
        default=config.port,
        help=f"Port to bind to (default: {config.port}, env: ATULYA_API_PORT)",
    )
    parser.add_argument(
        "--log-level",
    ...
    uvicorn_config = {
        "app": "atulya_api.server:app" if use_import_string else app,
        "host": args.host,
        "port": args.port,
        "log_level": args.log_level,
