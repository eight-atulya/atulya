...
DEFAULT_USERNAME = "atulya"
DEFAULT_PASSWORD = "atulya"
DEFAULT_DATABASE = "atulya"
...
def parse_pg0_url(db_url: str) -> tuple[bool, str | None, int | None]:
    ...
    """
    if db_url == "pg0":
        return True, "atulya", None

    if db_url.startswith("pg0://"):
        url_part = db_url[6:]  # Remove "pg0://"
        if ":" in url_part:
            instance_name, port_str = url_part.rsplit(":", 1)
            return True, instance_name or "atulya", int(port_str)
        else:
            return True, url_part or "atulya", None
