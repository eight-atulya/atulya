"""
OpenTelemetry distributed tracing instrumentation for Atulya API.
...
# Provider name mapping (Atulya internal -> GenAI semantic convention)
...
def create_operation_span(operation: str, bank_id: str | None = None):
    """
    Create a parent span for a Atulya operation (retain, reflect, consolidation, etc.).

    This creates the span hierarchy:
    - atulya.{operation} (parent)
    ...
    span_name = f"atulya.{operation}"
    span = _tracer.start_as_current_span(span_name)

    # Add operation-specific attributes
    if span and hasattr(span, "set_attribute"):
        span.set_attribute("atulya.operation", operation)
        if bank_id:
            span.set_attribute("atulya.bank_id", bank_id)
...
class LLMSpanRecorder:
    ...
    def record_llm_call(
        ...
        try:
            ...
            # Create span name: "atulya.{scope}" for consistency with parent spans
            # Model info is available in span attributes (gen_ai.request.model)
            if scope:
                span_name = f"atulya.{scope}"
            else:
            ...
            with self.tracer.start_as_current_span(
                ...
                # Add custom attributes for Atulya context
                span.set_attribute("atulya.scope", scope)
                span.set_attribute("atulya.provider.internal", provider)
