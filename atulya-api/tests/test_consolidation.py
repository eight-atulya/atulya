...
class TestObservationDrillDown:
    ...
    async def test_search_observations_returns_source_memory_ids(
        ...
        """
        from atulya_api.engine.reflect.tools import tool_expand, tool_search_observations
...
def test_observations_mission_config():
    ...
    original = os.getenv("ATULYA_API_OBSERVATIONS_MISSION")
    try:
        os.environ["ATULYA_API_OBSERVATIONS_MISSION"] = "Weekly sprint summaries only."
        clear_config_cache()
        config = _get_raw_config()
        assert config.observations_mission == "Weekly sprint summaries only."
        assert "observations_mission" in AtulyaConfig.get_configurable_fields()
    finally:
        if original is None:
            os.environ.pop("ATULYA_API_OBSERVATIONS_MISSION", None)
        else:
            os.environ["ATULYA_API_OBSERVATIONS_MISSION"] = original
        clear_config_cache()
...
async def test_consolidation_with_observations_mission(memory: "MemoryEngine", request_context):
    ...
    from atulya_api.config import _get_raw_config, clear_config_cache

    original = os.getenv("ATULYA_API_OBSERVATIONS_MISSION")
    try:
        os.environ["ATULYA_API_OBSERVATIONS_MISSION"] = (
