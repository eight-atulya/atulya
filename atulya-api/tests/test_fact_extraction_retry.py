...
def _make_config(llm_max_retries: int = 3, retain_llm_max_retries: int | None = None):
    """Build a minimal AtulyaConfig for fact extraction tests."""
    from atulya_api.config import AtulyaConfig

    cfg = MagicMock(spec=AtulyaConfig)
...
async def test_non_dict_json_all_retries_returns_empty():
    ...
    """
    from atulya_api.engine.retain.fact_extraction import _extract_facts_from_chunk
    ...
    with patch(
        "atulya_api.engine.retain.fact_extraction._build_extraction_prompt_and_schema",
