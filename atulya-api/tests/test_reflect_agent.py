...
from atulya_api.engine.reflect.agent import (
    ...
    _extract_reflect_answer_from_model_output,
...
class TestCleanAnswerText:
    ...
    def test_clean_channel_tokens_and_json_message(self):
        """Local models may emit <|channel|>...<|message|>{\"answer\":...} instead of plain text."""
        raw = (
    ...
    def test_extract_answer_from_message_json_only(self):
        """Bare JSON with answer after <|message|>."""
        s = '<|message|>{"answer":"Short reply."}'
        assert _extract_reflect_answer_from_model_output(s) == "Short reply."
...
class TestContextOverflowIntegration:
    ...
    async def test_reflect_completes_with_tiny_context_budget(self, memory, request_context):
        ...
        try:
            ...
            from atulya_api.config import get_config as _real_get_config
            ...
            with patch("atulya_api.engine.memory_engine.get_config", return_value=_TinyContextProxy()):
