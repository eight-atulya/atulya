...
from atulya_api import MemoryEngine, RequestContext
from atulya_api.engine.cross_encoder import CohereCrossEncoder, LocalSTCrossEncoder, ZeroEntropyCrossEncoder
from atulya_api.engine.embeddings import CohereEmbeddings, LocalSTEmbeddings, OpenAIEmbeddings
...
from atulya_api.extensions import TenantContext, TenantExtension
from atulya_api.migrations import ensure_embedding_dimension, run_migrations
...
def get_column_dimension(db_url: str, schema: str = "public", table: str = "memory_units") -> int | None:
    """Get embedding column dimension for a given table."""
    engine = create_engine(db_url)
    with engine.connect() as conn:
        result = conn.execute(
            text("""
...
class TestEmbeddingDimension:
    ...
    def test_dimension_change_empty_table(self, dimension_test_schema):
        """When tables are empty, dimensions can be changed for all embedding tables."""
        db_url, schema = dimension_test_schema
        ...
        # Verify dimension changed on both tables managed by ensure_embedding_dimension.
        mem_dim = get_column_dimension(db_url, schema, table="memory_units")
        model_dim = get_column_dimension(db_url, schema, table="mental_models")
        assert mem_dim == 768, f"Expected memory_units=768, got {mem_dim}"
        assert model_dim == 768, f"Expected mental_models=768, got {model_dim}"
...
def has_openai_api_key() -> bool:
    ...
    return bool(os.environ.get("ATULYA_API_EMBEDDINGS_OPENAI_API_KEY"))


def get_openai_api_key() -> str:
    """Get OpenAI API key from environment."""
    return os.environ.get("ATULYA_API_EMBEDDINGS_OPENAI_API_KEY", "")
...
def openai_embeddings():
    ...
    if not has_openai_api_key():
        pytest.skip("OpenAI API key not available (set ATULYA_API_EMBEDDINGS_OPENAI_API_KEY)")
...
class TestOpenAIEmbeddings:
    ...
    async def test_openai_embeddings_retain_recall(
        ...
        memory = MemoryEngine(
            db_url=db_url,
            memory_llm_provider=os.getenv("ATULYA_API_LLM_PROVIDER", "groq"),
            memory_llm_api_key=os.getenv("ATULYA_API_LLM_API_KEY"),
            memory_llm_model=os.getenv("ATULYA_API_LLM_MODEL", "openai/gpt-oss-120b"),
            memory_llm_base_url=os.getenv("ATULYA_API_LLM_BASE_URL") or None,
