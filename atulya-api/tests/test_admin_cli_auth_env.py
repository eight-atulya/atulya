...
def test_generate_auth_env_outputs_matching_admin_secret_and_pepper():
    ...
    assert values["ATULYA_CP_ADMIN_API_KEY"] == values["ATULYA_API_SUPERUSER_KEY"]
    assert values["ATULYA_API_KEY_HASH_PEPPER"].startswith("atulya_pepper_")
    assert values["ATULYA_API_KEY_HASH_PEPPER"] != values["ATULYA_API_SUPERUSER_KEY"]
    assert values["ATULYA_SIGNUP_MODE"] == "bootstrap"
