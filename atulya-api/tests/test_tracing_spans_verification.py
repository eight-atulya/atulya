...
async def test_recall_span_hierarchy(mock_tracer, memory, request_context):
    ...
    try:
        ...
        assert parent_call[0][0] == "atulya.recall"
        ...
        assert "atulya.bank_id" in recall_attrs
        assert recall_attrs["atulya.bank_id"] == bank_id
        assert "atulya.query" in recall_attrs
        assert "atulya.fact_types" in recall_attrs
        assert "atulya.thinking_budget" in recall_attrs
        assert "atulya.max_tokens" in recall_attrs
        ...
        if mock_tracer.start_span.called:
            ...
            assert "atulya.recall_embedding" in child_spans
            assert "atulya.recall_retrieval" in child_spans
            assert "atulya.recall_fusion" in child_spans
            assert "atulya.recall_rerank" in child_spans
...
async def test_recall_span_attributes(mock_tracer, memory, request_context):
    ...
    try:
        ...
        assert "atulya.bank_id" in attrs
        assert "atulya.query" in attrs
        assert "atulya.fact_types" in attrs
        assert "atulya.max_tokens" in attrs
        assert "atulya.thinking_budget" in attrs

        # Verify attribute values
        assert attrs["atulya.bank_id"] == bank_id
        assert "test query" in attrs["atulya.query"]
        assert attrs["atulya.max_tokens"] == 2048
