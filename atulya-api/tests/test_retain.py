...
def test_recall_result_model_empty_construction():
    ...
    """
    from atulya_api.engine.response_models import RecallResult
...
async def test_custom_extraction_mode():
    ...
    This test verifies that when ATULYA_API_RETAIN_EXTRACTION_MODE=custom and
    ATULYA_API_RETAIN_CUSTOM_INSTRUCTIONS is set, the fact extraction uses the
    custom guidelines while keeping structural parts intact.
    """
    import os
    from atulya_api import LLMConfig
    from atulya_api.engine.retain.fact_extraction import extract_facts_from_text
    from atulya_api.config import clear_config_cache, _get_raw_config

    # Save original env vars
    original_mode = os.getenv("ATULYA_API_RETAIN_EXTRACTION_MODE")
    original_instructions = os.getenv("ATULYA_API_RETAIN_CUSTOM_INSTRUCTIONS")

    try:
        # Set custom extraction mode with challenging language-specific guidelines
        os.environ["ATULYA_API_RETAIN_EXTRACTION_MODE"] = "custom"
        os.environ["ATULYA_API_RETAIN_CUSTOM_INSTRUCTIONS"] = """ONLY extract facts that are in ITALIAN language.
...
If the text contains both Italian and English content, extract ONLY the Italian facts."""
        ...
        if original_mode is not None:
            os.environ["ATULYA_API_RETAIN_EXTRACTION_MODE"] = original_mode
        else:
            os.environ.pop("ATULYA_API_RETAIN_EXTRACTION_MODE", None)

        if original_instructions is not None:
            os.environ["ATULYA_API_RETAIN_CUSTOM_INSTRUCTIONS"] = original_instructions
        else:
            os.environ.pop("ATULYA_API_RETAIN_CUSTOM_INSTRUCTIONS", None)
...
def test_retain_mission_injected_into_prompt():
    ...
    from atulya_api.engine.retain.fact_extraction import _build_extraction_prompt_and_schema
...
def test_retain_mission_config_loaded_from_env():
    ...
    import os
    from atulya_api.config import AtulyaConfig, _get_raw_config, clear_config_cache

    original = os.getenv("ATULYA_API_RETAIN_MISSION")
    try:
        os.environ["ATULYA_API_RETAIN_MISSION"] = "Only technical decisions."
        clear_config_cache()
        config = _get_raw_config()
        assert config.retain_mission == "Only technical decisions."
        assert "retain_mission" in AtulyaConfig.get_configurable_fields()
    finally:
        if original is None:
            os.environ.pop("ATULYA_API_RETAIN_MISSION", None)
        else:
            os.environ["ATULYA_API_RETAIN_MISSION"] = original
        clear_config_cache()
