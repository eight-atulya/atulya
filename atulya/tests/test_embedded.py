"""
Integration tests for AtulyaEmbedded client.
...
4. Method proxying to underlying AtulyaClient
5. Proper cleanup
...
def llm_config():
    ...
    provider = os.getenv("ATULYA_API_LLM_PROVIDER") or os.getenv("ATULYA_LLM_PROVIDER", "groq")
    api_key = os.getenv("ATULYA_API_LLM_API_KEY") or os.getenv("ATULYA_LLM_API_KEY", "")
    model = os.getenv("ATULYA_API_LLM_MODEL") or os.getenv("ATULYA_LLM_MODEL", "openai/gpt-oss-120b")

    if not api_key:
        pytest.skip("LLM API key not configured. Set ATULYA_API_LLM_API_KEY or ATULYA_LLM_API_KEY.")
...
def test_embedded_lazy_start(llm_config):
    """
    Test that AtulyaEmbedded starts server lazily on first use.
    """
    profile = f"test_lazy_{uuid.uuid4().hex[:8]}"
    bank_id = f"bank_{uuid.uuid4().hex[:8]}"

    # Create client - should NOT start server yet
    client = AtulyaEmbedded(profile=profile, log_level="info", **llm_config)
...
def test_embedded_context_manager(llm_config):
    """
    Test AtulyaEmbedded with context manager.
    """
    profile = f"test_ctx_{uuid.uuid4().hex[:8]}"
    bank_id = f"bank_{uuid.uuid4().hex[:8]}"

    # Use context manager
    with AtulyaEmbedded(profile=profile, log_level="info", **llm_config) as client:
...
def test_embedded_complete_workflow(llm_config):
    """
    Test complete workflow with AtulyaEmbedded.
...
def test_embedded_method_proxying(llm_config):
    """
    Test that all AtulyaClient methods are properly proxied.
...
def test_embedded_multiple_banks(llm_config):
    """
    Test that AtulyaEmbedded can work with multiple banks.
    """
...
def test_embedded_profile_isolation(llm_config):
    ...
    client1 = AtulyaEmbedded(profile=profile1, log_level="info", **llm_config)
    client2 = AtulyaEmbedded(profile=profile2, log_level="info", **llm_config)
...
def test_embedded_error_after_close(llm_config):
    """
    Test that using AtulyaEmbedded after close() raises an error.
    """
    ...
    with pytest.raises(RuntimeError, match="Cannot use AtulyaEmbedded after it has been closed"):
