"""
Embedded Atulya client with automatic daemon lifecycle management.

This module provides AtulyaEmbedded, a client that uses the same daemon
management interface as atulya-embed CLI, ensuring full compatibility.
...
Using context manager:
    ...
    with AtulyaEmbedded(profile="myapp") as client:
        client.retain(bank_id="alice", content="Alice loves AI")
...
from atulya_embed import get_embed_manager
...
class AtulyaEmbedded:
    """
    Atulya client with automatic daemon lifecycle management.

    This client uses the same daemon management interface as atulya-embed CLI,
    ...
    Profile data is stored in: ~/.pg0/instances/atulya-embed-{profile}/

    All methods from AtulyaClient are available:
    - retain(), retain_batch()
    - recall()
    - reflect()
    ...
    def __init__(
        ...
        self.config = {
            ...
            "ATULYA_API_LLM_API_KEY": llm_api_key,
            "ATULYA_API_LLM_MODEL": llm_model,
            "ATULYA_API_LOG_LEVEL": log_level,
            "ATULYA_EMBED_DAEMON_IDLE_TIMEOUT": str(idle_timeout),
        }

        if llm_base_url:
            self.config["ATULYA_API_LLM_BASE_URL"] = llm_base_url

        if database_url:
            self.config["ATULYA_EMBED_API_DATABASE_URL"] = database_url

        self._client: Optional[Atulya] = None
    ...
    def _ensure_started(self):
        ...
        with self._lock:
            ...
            if self._closed:
                raise RuntimeError("Cannot use AtulyaEmbedded after it has been closed")
            ...
            self._client = Atulya(base_url=daemon_url)
            self._started = True
    ...
    def __getattr__(self, name: str):
        """
        Proxy all method calls to the underlying Atulya client.

        This allows AtulyaEmbedded to expose all AtulyaClient methods
    ...
    def banks(self) -> BanksAPI:
        ...
        Example:
            ...
            embedded = AtulyaEmbedded(profile="myapp", ...)
    ...
    @property
    def client(self) -> Atulya:
        """
        Get the underlying Atulya client for direct access.
        ...
        directives, memories) or direct method calls on AtulyaEmbedded instead.

        Ensures daemon is started before returning the client.

        Returns:
            Atulya: The underlying client instance
