"""
Server module for running Atulya in a background thread.

Provides a simple way to start and stop the Atulya HTTP API server
...
class Server:
    """
    Atulya server that runs in a background thread.
        ...
        while time.time() - start_time < timeout:
            try:
                with socket.create_connection((self.host, self.port), timeout=1):
                    logger.info(f"Atulya server started at {self.url}")
                    return self
    ...
    def stop(self, timeout: float = 10.0) -> None:
        ...
        self._server = None
        self._thread = None
        logger.info("Atulya server stopped")
...
def start_server(
    ...
    """
    Start a Atulya server in a background thread.
    ...
    Example:
        ```python
        from atulya import start_server, Client
