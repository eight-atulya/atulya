...
The Atulya CLI provides command-line access to memory operations and bank management. All commands follow the [OpenAPI specification](/api-reference), so you can use `--help` on any command to see all available options.
...
atulya configure --api-url http://localhost:8888 --api-key your-api-key

# Or use environment variables (highest priority)
export ATULYA_API_URL=http://localhost:8888
export ATULYA_API_KEY=your-api-key
```
...
# Filter by fact type
atulya memory recall <bank_id> "query" --fact-type world,observation
...
```bash
atulya bank mission <bank_id> "I am a helpful AI assistant interested in technology"
```
