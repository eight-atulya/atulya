...
We built `@eight-atulya/atulya-chat` to give [Vercel Chat SDK](https://github.com/vercel/chat) bots persistent, per-user memory with a single handler wrapper. The integration works across Slack, Discord, Teams, Google Chat, GitHub, and Linear — no custom plumbing required.

## Installation

```bash
npm install @eight-atulya/atulya-chat
```
...
import { withAtulyaChat } from '@eight-atulya/atulya-chat';
import { streamText } from 'ai';
import { openai } from '@ai-sdk/openai';

const chat = new Chat({ connectors: [/* your connectors */] });
const atulya = new AtulyaClient({ apiKey: process.env.ATULYA_API_KEY });
...
### `withAtulyaChat(options, handler)`

`withAtulyaChat` wraps your existing Chat SDK handler and injects memory context automatically. It returns a standard handler `(thread, message) => Promise<void>` so it drops in without changing your handler signature.

#### Options

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `client` | `AtulyaClient` | *required* | Atulya client instance |
...
We inject a third `ctx` argument into your handler that exposes the full Atulya memory API scoped to the current user's bank:
...
chat.onNewMention(
  withAtulyaChat(
    { client: atulya, bankId: 'shared-team-memory' },
    async (thread, message, ctx) => {
      // ...
    }
  )
