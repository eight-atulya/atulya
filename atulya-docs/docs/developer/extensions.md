...
Extensions allow you to customize and extend Atulya behavior without modifying core code. They enable multi-tenancy, custom authentication, additional HTTP endpoints, and operation hooks.
...
```bash
ATULYA_API_TENANT_EXTENSION=atulya_api.extensions.builtin.supabase_tenant:SupabaseTenantExtension
ATULYA_API_TENANT_SUPABASE_URL=https://your-project.supabase.co
# Optional - only needed for legacy HS256 projects or health check
ATULYA_API_TENANT_SUPABASE_SERVICE_KEY=your-service-role-key
```

See the [source code](https://github.com/eight-atulya/atulya/blob/main/atulya-api/atulya_api/extensions/builtin/supabase_tenant.py) for complete configuration options and implementation details.
...
Adds custom HTTP endpoints under the `/ext/` path prefix. Useful for adding domain-specific APIs that integrate with Atulya's memory engine.
...
```bash
ATULYA_API_HTTP_EXTENSION=mypackage.ext:MyHttpExtension
```
...
```bash
ATULYA_API_OPERATION_VALIDATOR_EXTENSION=mypackage.validators:MyValidator
```
...
Registers additional MCP (Model Context Protocol) tools on the Atulya MCP server. Enables external packages to add custom tools without modifying core code.

**No built-in implementation** - implement your own to add custom MCP tools.

```bash
ATULYA_API_MCP_EXTENSION=mypackage.mcp:MyMCPExtension
```
...
```bash
ATULYA_API_<TYPE>_EXTENSION=mypackage.module:MyExtensionClass
```

Configuration is passed via prefixed environment variables:

```bash
ATULYA_API_<TYPE>_SOME_CONFIG=value
...
```python
import jwt
from atulya_api.extensions import TenantExtension, TenantContext, AuthenticationError
...
class JwtTenantExtension(TenantExtension):
    def __init__(self, config: dict[str, str]):
        ...
        if not self.jwt_secret:
            raise ValueError("ATULYA_API_TENANT_JWT_SECRET is required")
...
from atulya_api.extensions import HttpExtension
...
from atulya_api.extensions import MCPExtension
from atulya_api.engine import MemoryEngine
...
services:
  atulya-api:
    image: vectorize/atulya-api:latest
    volumes:
      - ./my_extensions:/app/my_extensions
    environment:
      - ATULYA_API_TENANT_EXTENSION=my_extensions.auth:JwtTenantExtension
      - ATULYA_API_TENANT_JWT_SECRET=${JWT_SECRET}
      - PYTHONPATH=/app
```
...
```dockerfile
FROM vectorize/atulya-api:latest
...
Install your extension package in the same Python environment as Atulya:
...
# Configure
export ATULYA_API_TENANT_EXTENSION=my_extensions.auth:JwtTenantExtension
export ATULYA_API_TENANT_JWT_SECRET=your-secret
...
Custom extensions that solve common use cases are welcome contributions to the Atulya project. If you've built an extension for:
...
Consider contributing it to the `atulya_api.extensions.builtin` package. Open an issue or pull request on [GitHub](https://github.com/eight-atulya/atulya) to discuss your extension.
