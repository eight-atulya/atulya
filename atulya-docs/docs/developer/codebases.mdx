...
<FeatureCardGrid
  cards={[
    ...
    },
    {
      ...
      title: 'Built for coders and tools',
      description:
        'ZIP import, GitHub import, refresh, files, symbols, impact, and approve are all exposed as typed APIs that fit real developer workflows.',
    },
    {
...
- **approval** decides when that parsed snapshot becomes memory-backed
- **Atulya memory** remains stable until a human accepts the new snapshot

```mermaid
flowchart LR
    A["Import ZIP or GitHub archive"] --> B["ASD mechanical parse"]
    B --> C["Reviewable snapshot"]
    C --> D["Repo map, symbol search, impact analysis"]
    C --> E["Explicit approval"]
    E --> F["Memory hydration"]
    F --> G["recall / reflect use approved codebase documents"]
```
