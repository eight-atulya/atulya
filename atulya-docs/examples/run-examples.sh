...
run_cli_examples() {
    ...
    # Check if atulya CLI is available
    if ! command -v atulya &> /dev/null; then
        echo -e "  ${YELLOW}SKIPPED (atulya CLI not installed)${NC}"
...
wait_for_server() {
    echo "Waiting for Atulya server at $ATULYA_URL..."
    for i in {1..30}; do
        if curl -s "$ATULYA_URL/health" > /dev/null 2>&1; then
