...
Requires a running Atulya server at `http://localhost:8888` (or set `ATULYA_API_URL`).
...
| Command | Reason |
|---------|--------|
| `atulya configure` | Requires interactive user input (prompts for API URL, credentials) |
| `atulya configure --show` | Displays sensitive configuration, not suitable for automated tests |
