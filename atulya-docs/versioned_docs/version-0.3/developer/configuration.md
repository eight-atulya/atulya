...
| Variable | Description | Default |
|----------|-------------|---------|
| `ATULYA_API_LLM_PROVIDER` | Provider: `openai`, `anthropic`, `gemini`, `groq`, `ollama`, `lmstudio` | `openai` |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `ATULYA_API_EMBEDDINGS_PROVIDER` | Provider: `local`, `tei`, `openai`, `cohere`, or `litellm` | `local` |
...
| `ATULYA_API_COHERE_API_KEY` | Cohere API key (shared for embeddings and reranker) | - |
...
| `ATULYA_API_LITELLM_API_BASE` | LiteLLM proxy base URL (shared for embeddings and reranker) | `http://localhost:4000` |
| `ATULYA_API_LITELLM_API_KEY` | LiteLLM proxy API key (optional, depends on proxy config) | - |
...
export ATULYA_API_EMBEDDINGS_PROVIDER=cohere
export ATULYA_API_COHERE_API_KEY=your-azure-api-key
...
export ATULYA_API_LITELLM_API_KEY=your-litellm-key  # optional
...
| Variable | Description | Default |
|----------|-------------|---------|
| `ATULYA_API_RERANKER_PROVIDER` | Provider: `local`, `tei`, `cohere`, `flashrank`, `litellm`, or `rrf` | `local` |
...
| `ATULYA_API_RERANKER_LITELLM_MODEL` | LiteLLM rerank model (use provider prefix, e.g., `cohere/rerank-english-v3.0`) | `cohere/rerank-english-v3.0` |
...
export ATULYA_API_COHERE_API_KEY=your-api-key  # shared with embeddings
...
| Variable | Description | Default |
|----------|-------------|---------|
| `ATULYA_API_OBSERVATION_MIN_FACTS` | Minimum facts about an entity before generating observations | `5` |
| `ATULYA_API_OBSERVATION_TOP_ENTITIES` | Max entities to process per retain batch | `5` |
...
| `ATULYA_API_RETAIN_EXTRACTION_MODE` | Fact extraction mode: `concise` (selective, fewer high-quality facts) or `verbose` (detailed, more facts) | `concise` |
| `ATULYA_API_RETAIN_EXTRACT_CAUSAL_LINKS` | Extract causal relationships between facts | `true` |
| `ATULYA_API_RETAIN_OBSERVATIONS_ASYNC` | Run entity observation generation asynchronously (after retain completes) | `false` |
...
Configuration for the local MCP server (`atulya-local-mcp` command).
...
| `ATULYA_API_WORKER_BATCH_SIZE` | Tasks to claim per poll cycle | `10` |
