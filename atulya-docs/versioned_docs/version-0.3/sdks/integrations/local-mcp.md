...
Atulya provides a fully local MCP server that runs entirely on your machine with an embedded PostgreSQL database. No external server or database setup required.
...
```bash
curl -fsSL https://atulya.eightengine.com/get-mcp | bash -s -- \
  --app claude-desktop \
  --set ATULYA_API_LLM_API_KEY=sk-...
```

This script will:
1. Install [uv](https://docs.astral.sh/uv/) if not already installed
2. Configure Claude Desktop to use the Atulya MCP server
...
```json
{
  "mcpServers": {
    "atulya": {
      "command": "uvx",
      "args": ["--from", "atulya-api", "atulya-local-mcp"],
      "env": {
        "ATULYA_API_LLM_API_KEY": "sk-..."
      }
    }
  }
}
...
```json
{
  "mcpServers": {
    "atulya": {
      ...
      "env": {
        "ATULYA_API_LLM_API_KEY": "sk-...",
        "ATULYA_API_MCP_LOCAL_BANK_ID": "my-personal-memory"
      }
    }
  }
}
...
All standard [Atulya configuration variables](/developer/configuration) are supported.

### Local MCP Specific

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `ATULYA_API_MCP_LOCAL_BANK_ID` | No | `mcp` | Memory bank ID to use |
| `ATULYA_API_MCP_INSTRUCTIONS` | No | - | Additional instructions appended to both `retain` and `recall` tools |
...
curl -fsSL https://atulya.eightengine.com/get-mcp | bash -s -- \
  --app claude-desktop \
  --set ATULYA_API_LLM_API_KEY=sk-... \
  --set ATULYA_API_MCP_INSTRUCTIONS="Also store every action you take, code you write, and files you modify."
```
...
2. **Initializes the Atulya memory engine** with local embeddings
3. **Connects via stdio** to Claude Code using the MCP protocol

Data is persisted in the pg0 data directory (`~/.pg0/atulya-mcp/`), so your memories survive restarts.

## Troubleshooting

### "ATULYA_API_LLM_API_KEY required"
