...
Atulya automatically detects the language of your input and responds in the same language. This means facts, entities, and reflect responses are preserved in their original language without translation to English.
...
When you retain content or reflect on a query, Atulya:
...
When you retain content in any language, Atulya extracts and stores facts in that same language.
...
Atulya handles mixed-language content gracefully, preserving both languages where appropriate.
...
**Atulya's multilingual support depends entirely on your LLM's language capabilities.** Atulya instructs the LLM to detect the input language and respond in that same language. If your LLM supports a language, Atulya will work with it.
...
**To verify support for your target language**, test your LLM directly with content in that language. If the LLM can understand and generate text in the language, Atulya will preserve it correctly.
...
```bash
# In your .env file
ATULYA_API_EMBEDDINGS_LOCAL_MODEL=BAAI/bge-m3
```
...
```bash
# In your .env file
ATULYA_API_RERANKER_LOCAL_MODEL=BAAI/bge-reranker-v2-m3
```
