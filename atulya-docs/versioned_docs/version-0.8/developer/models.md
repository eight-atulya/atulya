...
Each provider has a recommended default model that's used when `ATULYA_API_LLM_MODEL` is not explicitly set. This makes configuration simpler - just specify the provider and get a sensible default:
...
You can override the default by explicitly setting `ATULYA_API_LLM_MODEL`:
```bash
...
export ATULYA_API_RETAIN_LLM_PROVIDER=anthropic
```

### Using Other Models

Other LLM models not listed above may work with Atulya, but they must support **at least 65,000 output tokens** to ensure reliable fact extraction. If you need support for a specific model that doesn't meet this requirement, please [open an issue](https://github.com/eight-atulya/atulya/issues) to request an exception.
...
export ATULYA_API_RETAIN_MAX_COMPLETION_TOKENS=32000

# For models that support 16k output tokens
export ATULYA_API_RETAIN_MAX_COMPLETION_TOKENS=16000
```

**Important:** `ATULYA_API_RETAIN_MAX_COMPLETION_TOKENS` must be greater than `ATULYA_API_RETAIN_CHUNK_SIZE` (default: 3000). The system will validate this on startup and provide an error message if the configuration is invalid.
:::
...
# export ATULYA_API_LLM_VERTEXAI_SERVICE_ACCOUNT_KEY=/path/to/key.json
```
...
Use your ChatGPT Plus or Pro subscription for Atulya without separate OpenAI Platform API costs.
...
4. **Configure Atulya:**
   ...
   # export ATULYA_API_LLM_MODEL=gpt-5.1-codex  # defaults to gpt-5.2-codex
...
Use your Claude Pro or Max subscription for Atulya without separate Anthropic API costs.
...
- Anthropic's [Agent SDK documentation](https://docs.claude.com/en/api/agent-sdk/overview)
  ...
  their products. Atulya does **not** perform any login on your behalf — it uses
...
- In January 2026, Anthropic [enforced restrictions](https://paddo.dev/blog/anthropic-walled-garden-crackdown/)
  ...
  targeted tools that **spoofed the Claude Code client identity** — Atulya uses the
...
For production or team use, we recommend using `ATULYA_API_LLM_PROVIDER=anthropic` with
...
|----------|-------------|----------|
| `ATULYA_API_LLM_VERTEXAI_PROJECT_ID` | Your GCP project ID | Yes |
| `ATULYA_API_LLM_VERTEXAI_REGION` | GCP region (e.g., `us-central1`) | No (default: `us-central1`) |
| `ATULYA_API_LLM_VERTEXAI_SERVICE_ACCOUNT_KEY` | Path to service account JSON key file | No (uses ADC if not set) |
...
1. **Application Default Credentials (ADC)** - Recommended for development
   ...
   export ATULYA_API_LLM_PROVIDER=vertexai
   export ATULYA_API_LLM_MODEL=gemini-2.0-flash-001
   export ATULYA_API_LLM_VERTEXAI_PROJECT_ID=your-project-id
   ```

2. **Service Account Key** - Recommended for production
   ```bash
   # Create service account and download key
   gcloud iam service-accounts create atulya-api
   gcloud projects add-iam-policy-binding your-project-id \
     --member="serviceAccount:atulya-api@your-project-id.iam.gserviceaccount.com" \
     --role="roles/aiplatform.user"
   gcloud iam service-accounts keys create key.json \
     --iam-account=atulya-api@your-project-id.iam.gserviceaccount.com
...
export ATULYA_API_RERANKER_ZEROENTROPY_MODEL=zerank-2  # default, can omit
