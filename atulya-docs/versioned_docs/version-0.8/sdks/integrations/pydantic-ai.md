...
Persistent memory tools for [Pydantic AI](https://ai.pydantic.dev/) agents via Atulya. Give your agents long-term memory with retain, recall, and reflect — all async-native with no thread-pool hacks.
...
```bash
pip install atulya-pydantic-ai
```
...
- **`atulya_retain`** — Store information to long-term memory
- **`atulya_recall`** — Search long-term memory for relevant facts
- **`atulya_reflect`** — Synthesize a reasoned answer from memories
...
|---|---|---|
| `bank_id` | *required* | Atulya memory bank ID |
| `client` | `None` | Pre-configured Atulya client |
| `atulya_api_url` | `None` | API URL (used if no client provided) |
...
| Parameter | Default | Description |
|---|---|---|
| `atulya_api_url` | Production API | Atulya API URL |
| `api_key` | `ATULYA_API_KEY` env | API key for authentication |
