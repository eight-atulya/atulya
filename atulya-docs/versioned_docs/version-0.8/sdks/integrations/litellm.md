...
1. **Memory Retrieval** - Atulya is queried for relevant memories based on the conversation
2. **Prompt Injection** - Memories are injected into the system message
3. **LLM Call** - The enriched prompt is sent to the LLM
4. **Conversation Storage** - The conversation is stored to Atulya for future recall
...
atulya_litellm.configure(
    ...
    mission="This agent...",       # Instructions guiding what Atulya should remember
