...
Official Python client for the Atulya API.
...
The `atulya-all` package includes embedded PostgreSQL, HTTP API server, and client:
...
If you already have a Atulya server running:
...
`AtulyaEmbedded` provides the simplest way to use Atulya in Python. It automatically manages a background server for you - no manual setup required:
...
A profile is an isolated Atulya environment. Each profile gets its own PostgreSQL database (stored in `~/.pg0/instances/atulya-embed-{profile}/`) and its own API server. Use different profiles to separate environments (dev/prod), applications, or users.

**When to Use AtulyaEmbedded**

Use `AtulyaEmbedded` when you want the server to start automatically and manage itself. Use `AtulyaServer` when you need explicit control over server lifecycle (e.g., testing where you want immediate startup/shutdown).

**Advanced Operations**

`AtulyaEmbedded` provides organized API namespaces for advanced operations. Each method call automatically ensures the daemon is running:

```python
from atulya import AtulyaEmbedded
import os

embedded = AtulyaEmbedded(
    profile="myapp",
    llm_provider="openai",
    llm_api_key=os.environ["OPENAI_API_KEY"],
)
...
```python
from atulya import AtulyaClient

client = AtulyaClient(
    base_url="http://localhost:8888",  # Atulya API URL
...
# Organized API access (same as AtulyaEmbedded)
...
Both `AtulyaClient` and `AtulyaEmbedded` provide the same organized API namespaces (`banks`, `mental_models`, `directives`, `memories`) for consistent developer experience.
...
with Atulya(base_url="http://localhost:8888") as client:
