...
[**View source on GitHub →**](https://github.com/eight-atulya/atulya-cookbook/tree/main/applications/stancetracker)
:::


An AI-powered application that tracks political candidates' stances on issues over time using Atulya memory system and web scraping.
...
### Memory System (Atulya Integration)

This app uses the Atulya memory system from `github.com/eight-atulya/atulya`:
...
- **Memory**: Atulya (from github.com/eight-atulya/atulya)
...
1. **Atulya API** running (from github.com/eight-atulya/atulya)
2. **API Keys**:
...
```env
# Atulya API (from github.com/eight-atulya/atulya)
...
Clone and run Atulya from github.com/eight-atulya/atulya:

```bash
# Clone and run github.com/eight-atulya/atulya
cd /path/to/atulya
cargo run --bin atulya-server
```

Verify Atulya is running at `http://localhost:8888`
...
## Atulya Integration Examples

### 1. Storing Memories

```typescript
// Store web scraping results
await atulyaClient.retain(bankId, articleContent, {
...
const results = await atulyaClient.recall(bankId, query, {
  budget: 'high',
  maxTokens: 8192
});
```
...
const response = await atulyaClient.reflect(bankId,
...
│   ├── atulya-client.ts    # Atulya API client
...
- **Rate Limits**: Respect API rate limits for Tavily, Atulya, and LLM providers
...
- Atulya documentation: `github.com/eight-atulya/atulya/README.md`
