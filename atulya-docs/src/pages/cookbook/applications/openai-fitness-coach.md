...
# OpenAI Agent + Atulya Memory Integration


:::info Complete Application
This is a complete, runnable application demonstrating Atulya integration.
[**View source on GitHub →**](https://github.com/eight-atulya/atulya-cookbook/tree/main/applications/openai-fitness-coach)
:::


A fitness coach example demonstrating how to use **OpenAI Agents** with **Atulya as a memory backend**.
...
- **Atulya** providing sophisticated memory storage & retrieval
...
|-----------|---------------|-------------------|
| **Conversation** | Atulya `/think` endpoint | OpenAI Assistant API |
| **Memory** | Atulya (built-in) | Atulya (via function calling) |
| **LLM** | Configured in Atulya | OpenAI GPT-4 |
| **Opinion Formation** | Automatic in `/think` | Explicit via `store_memory(type="opinion")` |
| **Best For** | Atulya-native apps | Integrating memory into existing OpenAI agents |
...
2. **Atulya API running**
   ```bash
   # Follow Atulya setup instructions to start the API
...
Each function makes API calls to Atulya to fetch relevant memories.
...
3. Functions fetch data from Atulya
...
# Function retrieves from Atulya:
{
  "results": [
...
3. Retrieve data from Atulya
4. Generate a personalized response

## Memory Types Retrieved

The OpenAI Agent can retrieve different memory types from Atulya:
...
**To add Atulya memory to your own OpenAI Agent:**

1. Define function tools that call Atulya API
2. Register them with your OpenAI Assistant
3. Implement function handlers to execute Atulya queries
4. Let OpenAI Assistant decide when to retrieve memories

The key benefit: **Separation of concerns**
- OpenAI = Conversation logic
- Atulya = Memory storage, retrieval, temporal queries, entity linking

## When to Use This vs. Standard Atulya

**Use OpenAI + Atulya (this example) when:**
...
- You want to combine Atulya with other OpenAI features

**Use Atulya directly when:**
...
- `memory_tools.py` - Function calling tools that bridge to Atulya API
...
**"Agent not found"**
- Make sure the Atulya fitness-coach agent exists

**"Connection refused"**
- Make sure Atulya API is running on localhost:8888
...
- Atulya (temporal-semantic memory)
