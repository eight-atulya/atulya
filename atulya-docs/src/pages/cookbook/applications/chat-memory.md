...
[**View source on GitHub →**](https://github.com/eight-atulya/atulya-cookbook/tree/main/applications/chat-memory)
:::


A demo chat application that uses Groq's `qwen/qwen3-32b` model with Atulya for persistent per-user memory.
...
First, start the Atulya API server using Docker:

```bash
export GROQ_API_KEY=your_groq_api_key_here

# Start Atulya with Groq as the LLM provider
docker run --rm -it --pull always -p 8888:8888 -p 9999:9999 \
  -e ATULYA_API_LLM_PROVIDER=groq \
  -e ATULYA_API_LLM_API_KEY=$GROQ_API_KEY \
  -e ATULYA_API_LLM_MODEL="openai/gpt-oss-20b" \
...
echo "ATULYA_API_URL=http://localhost:8888" >> .env.local
```
...
2. **Memory Bank Creation**: First message creates a personal memory bank in Atulya
...
Next.js API Route (/api/chat)
     ↓
Atulya.recall() → Get relevant memories
     ↓
Groq API → Generate response with memory context
     ↓
Atulya.retain() → Store conversation
     ↓
Response to User
```
...
4. **Memory Verification**: Visit the Atulya Control Plane at http://localhost:9999 to see stored memories
