...
[**Open in GitHub →**](https://github.com/eight-atulya/atulya-cookbook/blob/main/notebooks/04-litellm-memory-demo.ipynb)
:::


This notebook demonstrates how to add persistent memory to any LLM app using the `atulya-litellm` package. Memory storage and injection happen automatically via LiteLLM callbacks - no manual memory management needed!
...
The `atulya-litellm` package hooks into LiteLLM's callback system to:
...
```python
!pip install atulya-litellm litellm nest_asyncio python-dotenv -U -q
```
...
- Store conversations to Atulya after the response
...
print("Atulya memory integration enabled!")
```

## Conversation 1: User Introduces Themselves

In this first conversation, the user shares some information about themselves. This will be automatically stored to Atulya memory.
...
# Use atulya_litellm.completion() directly
response_1 = atulya_litellm.completion(
    model="gpt-4o-mini",
    messages=[
...
print("\n(Conversation automatically stored to Atulya)")
```

## Wait for Memory Processing

Atulya needs a few seconds to process and extract facts from the conversation.
...
response_2 = atulya_litellm.completion(
    model="gpt-4o-mini",
    messages=[
        {"role": "system", "content": "You are a helpful assistant."},
