...
[**Open in GitHub →**](https://github.com/eight-atulya/atulya-cookbook/blob/main/notebooks/05-tool-learning-demo.ipynb)
:::


This notebook demonstrates how Atulya helps an LLM learn which tool to use when tool names are ambiguous. Without memory, the LLM might randomly select between similarly-named tools. With Atulya, it learns from past interactions and consistently makes the correct choice.
...
**Without Atulya:** The LLM guesses randomly based on vague descriptions  
**With Atulya:** The LLM learns from feedback which channel handles what

## Prerequisites

Make sure you have Atulya running:
...
```python
!pip install atulya-litellm atulya-client litellm nest_asyncio python-dotenv -U -q
```
...
def make_routing_request(user_request: str, use_atulya: bool, bank_id: Optional[str] = None):
...
def store_feedback(bank_id: str, request: str, correct_tool: str, request_type: str):
    ...
    client = Atulya(base_url=ATULYA_API_URL, timeout=60.0)
...
## Phase 1: Without Atulya (No Memory)
...
```python
print("=" * 60)
print("PHASE 1: WITHOUT ATULYA (No Memory)")
print("=" * 60)
...
for i, scenario in enumerate(TEST_SCENARIOS[:3], 1):
    ...
    tool_name = make_routing_request(scenario['request'], use_atulya=False)
...
print("\nWaiting 15 seconds for Atulya to process memories...")
time.sleep(15)
print("Done!")
```

## Phase 3: With Atulya (Memory-Augmented)

The LLM now has access to learned routing knowledge via Atulya. It should route requests correctly based on past feedback.


```python
print("=" * 60)
print("PHASE 3: WITH ATULYA (Memory-Augmented)")
print("=" * 60)
...
print(f"Phase 3 (With Atulya): {phase3_accuracy:.0f}% accuracy")

improvement = phase3_accuracy - phase1_accuracy
if improvement > 0:
    print(f"\n🎉 Improvement: +{improvement:.0f}% accuracy with Atulya!")
elif improvement == 0:
...
print("KEY INSIGHT")
print("=" * 60)
print("Atulya allows the LLM to learn from experience which tool")
...
```python
atulya_litellm.cleanup()

# Optional: delete the bank
import requests
response = requests.delete(f"{ATULYA_API_URL}/v1/default/banks/{bank_id}")
