...
# Movie Recommendation Assistant with Atulya Memory


:::tip Run this notebook
This recipe is available as an interactive Jupyter notebook.
[**Open in GitHub →**](https://github.com/eight-atulya/atulya-cookbook/blob/main/notebooks/movie_recommendation.ipynb)
:::
...
## Prerequisites
- OpenAI API key
- Atulya running locally via Docker (see setup below)

## Start Atulya Locally

Before running this notebook, start Atulya in a terminal:
...
```python
!pip install -q atulya-client openai nest-asyncio
```

## 2. Configure OpenAI API Key

Enter your OpenAI API key when prompted (used by both Atulya and the demo).
...
# Set OpenAI API key (used by both Atulya and the demo)
...
# Initialize Atulya client (connects to local Docker instance)
atulya = Atulya(
    base_url=os.getenv("ATULYA_BASE_URL", "http://localhost:8888"),
)
...
These functions demonstrate the three core Atulya operations:
...
Use `reflect()` to synthesize what Atulya has learned about your movie tastes.
...
Close the Atulya client connection.
