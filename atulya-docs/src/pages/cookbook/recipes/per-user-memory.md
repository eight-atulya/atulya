...
[**Open in GitHub →**](https://github.com/eight-atulya/atulya-cookbook/blob/main/notebooks/02-per-user-memory.ipynb)
:::
...
```python
!pip install atulya-client nest_asyncio openai python-dotenv -U
```
...
def on_user_signup(user_id: str):
    ...
    print(f"View bank: {ATULYA_UI_URL}/banks/user-{user_id}?view=documents")
```

## 2. Manage Conversation Sessions

Use `document_id` to group messages belonging to the same conversation. When you retain with the same `document_id`, Atulya replaces the previous version (upsert behavior), keeping the memory up-to-date as the conversation evolves.
...
print(f"\nView documents: {ATULYA_UI_URL}/banks/user-alice?view=documents")
```
...
Atulya automatically extracts and connects:
...
response = requests.delete(f"{ATULYA_API_URL}/v1/default/banks/user-alice")
