...
`atulya-litellm` hooks into LiteLLM to intercept every LLM call. Before the call, it retrieves relevant memories from Atulya and injects them into the prompt. After the call, it stores the conversation for future retrieval.

```
Your code ──→ atulya_litellm.completion()
                    │
                    ├─→ 1. Recall/Reflect from Atulya (relevant memories)
                    ├─→ 2. Inject memories into prompt
                    ├─→ 3. Forward to LLM (any provider via LiteLLM)
                    ├─→ 4. Store conversation to Atulya
                    └─→ 5. Return response
```
...
The `atulya_query` tells Atulya what to search for in memory. The response comes back with context from past conversations — the LLM knows about the user's project, their preferences, their tech stack.
...
# Azure OpenAI
atulya_litellm.completion(model="azure/gpt-4", messages=messages, atulya_query=query)

# AWS Bedrock
atulya_litellm.completion(model="bedrock/anthropic.claude-3", messages=messages, atulya_query=query)

# Google Vertex AI
atulya_litellm.completion(model="vertex_ai/gemini-pro", messages=messages, atulya_query=query)
```
...
Best for natural, conversational context. The `reflect_context` parameter shapes how Atulya reasons about the memories without affecting what it retrieves.
...
```python
from atulya_litellm import recall
...
```python
from atulya_litellm import reflect
...
```python
from atulya_litellm import retain
...
response = atulya_litellm.completion(
    ...
    atulya_bank_id="other-agent",          # Different bank for this call
    atulya_budget="high",                  # More thorough retrieval
    atulya_reflect_context="Currently helping with onboarding",
)
```

Any default can be overridden with a `atulya_*` prefix.
...
Tell Atulya what kind of knowledge the bank should build. This shapes how memories are consolidated into mental models. Pass `mission` and `bank_name` to `configure()`:
...
- `atulya-litellm` gives any LLM persistent memory across conversations
...
- **Try it locally**: `pip install atulya-all atulya-litellm` and run the quick start above
