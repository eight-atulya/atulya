...
- OpenClaw's built-in memory is file-based -- markdown files on disk with SQLite vector search. It works, but the agent has to decide what to remember. Atulya automates the entire pipeline.
- Atulya is open source and runs locally by default. Your conversations, extracted knowledge, and memory store never leave your machine unless you choose otherwise.
- One plugin install, three commands to set up. The `atulya-embed` daemon bundles the full memory engine (API + PostgreSQL) into a single process.
- Memories auto-inject into context before each response -- no tool calls, no retrieval logic to write.
- For teams, an external API mode connects to a shared Atulya server so multiple OpenClaw instances can share memory.
...
[Atulya](https://github.com/eight-atulya/atulya) is an open-source memory engine that replaces OpenClaw's memory layer with automated, structured knowledge extraction. The key differences from the built-in system:

**Automatic capture, not manual.** Every conversation is captured after each turn without the agent needing to decide what's worth remembering. Atulya extracts facts, entities, and relationships in the background -- the model doesn't need to be prompted to "save this."

**Structured knowledge, not flat text.** Instead of appending lines to a Markdown file, Atulya extracts discrete facts ("production database runs on port 5433"), tracks entities (people, services, projects), and maps relationships between them ("auth service depends on Redis for sessions").

**Auto-recall, not tool-based retrieval.** OpenClaw's built-in memory exposes a `memory_search` tool that the agent can call, but models don't use search tools consistently. Atulya sidesteps this entirely by injecting relevant memory into context *before* every agent response. The agent doesn't need to know the memory system exists.

**Feedback loop prevention.** When memories are injected into context before a response, they become part of the conversation. Without care, those injected memories would get re-stored and re-extracted as new facts, causing exponential growth and duplicates. The plugin automatically strips its own `<atulya_memories>` tags before retention, preventing this loop.

**Local-first, open source.** Atulya runs through `atulya-embed`, a daemon that bundles the memory API and a PostgreSQL instance into a single process on your machine. No data leaves your environment. The entire codebase is open source.

```
┌─────────────────┐       ┌────────────────────────────────┐
│    OpenClaw      │       │    atulya-embed daemon      │
│    Gateway       │──────▶│    ┌──────────┐ ┌───────────┐ │
│                  │◀──────│    │ Memory   │ │PostgreSQL │ │
...
Atulya needs an LLM for memory extraction. This is separate from your agent's primary model -- it runs in the background and handles fact/entity/relationship extraction.
...
export ATULYA_API_LLM_MODEL=xiaomi/mimo-v2-flash
export ATULYA_API_LLM_API_KEY=YOUR_API_KEY
...
You should see output confirming the install and that Atulya takes over the memory slot:

```
Exclusive slot "memory" switched from "memory-core" to "atulya-openclaw".
Installed plugin: atulya-openclaw
```

This confirms Atulya is replacing OpenClaw's built-in memory, not running alongside it.
...
The Atulya daemon starts automatically on port 9077. You should see confirmation in the gateway output:
...
If you want to browse what your agent has learned, the Atulya daemon includes a web UI:
...
The default local daemon is ideal for a single OpenClaw instance. But if you're running multiple instances -- say, one on your laptop and one on a server -- or if your team wants shared agent memory, the plugin supports connecting to a remote Atulya API server.

Configure via environment variables:

```bash
export ATULYA_EMBED_API_URL=https://your-atulya-server.example.com
export ATULYA_EMBED_API_TOKEN=YOUR_API_TOKEN
openclaw gateway
```
  ...
  "plugins": {
    "entries": {
      "atulya-openclaw": {
        "enabled": true,
        "config": {
          "atulyaApiUrl": "https://your-atulya-server.example.com",
          "atulyaApiToken": "YOUR_API_TOKEN"
        }
      }
    }
  }
...
```bash
# [Atulya] External API mode enabled: https://your-atulya-server.example.com
...
**The embedded PostgreSQL needs system libraries.** `atulya-embed` bundles PostgreSQL via [pg0](https://github.com/eight-atulya/pg0). On minimal Docker images (e.g. `ubuntu:latest`), you'll need to install `libxml2` and `libreadline` before the daemon can start:
...
**When Atulya is the better choice:**
...
The local daemon is simpler and keeps everything on one machine -- the right default for personal use. External API mode adds network latency but enables shared memory and survives machine restarts. Since Atulya is open source, you can self-host the server on your own infrastructure and keep the same data ownership guarantees.

## Recap

OpenClaw's built-in memory is file-based and transparent, but it depends on the agent deciding what to remember and when to search. Atulya replaces this with automated extraction and auto-recall -- conversations are captured, facts are extracted in the background, and relevant knowledge is injected into context before every response.
...
And because Atulya is open source and local-first, you keep the same data ownership model that makes OpenClaw compelling in the first place.

## Next Steps

- Install the plugin and have a few conversations across different channels. Then open the web UI (`uvx atulya-embed@latest -p openclaw ui`) to see what was captured.
...
- Browse the [Atulya source on GitHub](https://github.com/eight-atulya/atulya) to understand the extraction pipeline.
- Read the [full integration docs](https://atulya.eightengine.com/sdks/integrations/openclaw) for the complete configuration reference.
- If you're running multiple OpenClaw instances, try external API mode with a self-hosted Atulya server for shared memory.
