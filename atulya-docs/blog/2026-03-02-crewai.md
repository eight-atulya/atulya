...
That's what the `atulya-crewai` package does. It implements CrewAI's `Storage` interface using Atulya's memory engine, so your crews remember everything -- across runs, across days, across weeks.
...
CrewAI Crew
  └─ ExternalMemory
       └─ AtulyaStorage (implements Storage interface)
            ├─ save()   → Atulya retain (extract facts, entities, relationships)
            ├─ search() → Atulya recall (semantic + graph + temporal retrieval)
            └─ reset()  → Atulya delete_bank + recreate
```

CrewAI calls `save()` after each task completes and `search()` before each task starts. You don't manage the lifecycle -- CrewAI drives it, Atulya stores it.

Under the hood, Atulya does more than store text. It extracts structured facts, identifies entities, builds a knowledge graph, and runs multi-strategy retrieval (semantic search, BM25, graph traversal, temporal ranking) with cross-encoder reranking.
...
> **Note:** You can also use [Atulya Cloud](https://ui.atulya.eightengine.com/signup) and skip the self-hosted setup entirely.
...
This pulls in `atulya-client` and `crewai` as dependencies.
...
That's it. After `kickoff()`, every task output is retained in Atulya. Next time you run this crew, it recalls relevant prior work before starting each task.
...
CrewAI's Storage interface has `save` and `search`. But Atulya also supports `reflect` -- a synthesis operation that reasons across all relevant memories instead of returning raw facts.
...
researcher = Agent(
    ...
    backstory="You are a senior researcher. Use the atulya_reflect tool to review what you already know before starting new research.",
    tools=[reflect_tool],
    llm="openai/gpt-4o-mini",
)
```

When the agent calls `atulya_reflect`, it gets a synthesized, reasoned response that draws on the full knowledge graph -- not just the top-k vector matches.
...
**2. Large task outputs.** CrewAI passes the full task output to `save()`. If your tasks produce very long outputs, Atulya handles the chunking, but retain latency increases. Set a reasonable `expected_output` length in your task definitions.
...
**4. Async event loop conflicts.** CrewAI runs inside an async event loop. The integration handles this transparently via a dedicated thread pool, but if you're also doing async work in custom tools, avoid calling `atulya-client` directly from the same event loop. Use the `AtulyaStorage` and `AtulyaReflectTool` abstractions instead.

---

## Recap

- `atulya-crewai` gives CrewAI agents persistent, compounding memory
...
- `AtulyaReflectTool` adds on-demand synthesis for deeper reasoning
...
- **Try it locally**: `pip install atulya-all atulya-crewai` and run the example above
- **Use Atulya Cloud**: Skip self-hosting with a [free account](https://ui.atulya.eightengine.com/signup)
...
- **Inspect memories in the web UI**: Run `atulya-control-plane` locally or use the cloud dashboard to browse facts, entities, and mental models
