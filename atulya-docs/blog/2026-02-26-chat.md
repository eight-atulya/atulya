...
- `@eight-atulya/atulya-chat` adds persistent memory with one wrapper function
...
[`@eight-atulya/atulya-chat`](https://atulya.eightengine.com/sdks/integrations/chat) adds a `withAtulyaChat()` wrapper to any Chat SDK handler. It automatically recalls relevant memories before your handler runs and retains conversations after.

```
Slack message ─────┐
                    ├─→ withAtulyaChat() ─→ recall ─→ your handler ─→ retain
Discord message ───┘                            │
                                                ▼
                                         Atulya API
                                         (shared memory)
...
That's the whole handler. The wrapper takes care of recalling memories, formatting them for the LLM, and retaining conversations. You just write the logic. See the [full API reference](https://atulya.eightengine.com/sdks/integrations/chat) for all configuration options.
...
Atulya isn't a simple vector store. When the bot retains a conversation, Atulya's pipeline:
...
The Vercel Chat SDK supports Slack, Discord, Microsoft Teams, Google Chat, GitHub, and Linear. The `withAtulyaChat()` wrapper works with all of them -- same code, same memory, any combination of platforms.

---

## Try It

The full working example is in the [Atulya Cookbook](https://github.com/eight-atulya/atulya-cookbook/tree/main/applications/chat-sdk-multi-platform). It includes:

- Complete bot with Slack + Discord adapters
- LLM-powered responses with Vercel AI SDK
- Shared Atulya memory bank
- Discord Gateway route setup
- Environment configuration and setup instructions

Start Atulya locally or use [Atulya Cloud](https://ui.atulya.eightengine.com/signup), clone the cookbook, and you'll have a working cross-platform bot with memory in minutes.

---

## Next Steps

- **Read the docs**: [Chat SDK integration guide](https://atulya.eightengine.com/sdks/integrations/chat) covers the full API, configuration options, and advanced patterns
- **Clone the cookbook**: [chat-sdk-multi-platform](https://github.com/eight-atulya/atulya-cookbook/tree/main/applications/chat-sdk-multi-platform) has the full working example
- **Try Atulya Cloud**: Skip self-hosting with a [free account](https://ui.atulya.eightengine.com/signup)
