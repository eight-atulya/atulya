...
Atulya 0.4.0 introduces a hierarchical learning system:
...
In Atulya 0.3.0, we had two separate systems for synthesized knowledge:
...
After every `retain()` call, Atulya's consolidation engine runs automatically:
...
Mental models are **saved reflect responses** that you curate for your memory bank. When you create a mental model, Atulya runs a reflect operation with your source query and stores the result. During future reflect calls, these pre-computed summaries are checked first.
