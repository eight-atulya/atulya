#!/bin/bash
#
# Smoke test for atulya-embed CLI with daemon mode
...
API_DIR="$(cd "$SCRIPT_DIR/../atulya-api" && pwd)"

echo "=== Atulya Embed Smoke Test (Daemon Mode) ==="

# Check required environment (load from config if not set)
if [ -f ~/.atulya/config.env ]; then
    source ~/.atulya/config.env
fi

# vertexai uses GCP service account credentials instead of an API key
if [ -z "$ATULYA_API_LLM_API_KEY" ] && [ -z "$OPENAI_API_KEY" ]; then
    if [ "${ATULYA_API_LLM_PROVIDER}" != "vertexai" ]; then
        echo "Error: ATULYA_API_LLM_API_KEY or OPENAI_API_KEY is required"
        exit 1
    fi
fi
...
echo ""
echo "Environment:"
echo "  ATULYA_API_LLM_PROVIDER: ${ATULYA_API_LLM_PROVIDER:-not set}"
echo "  ATULYA_API_LLM_MODEL: ${ATULYA_API_LLM_MODEL:-not set}"
echo "  ATULYA_API_LLM_API_KEY: ${ATULYA_API_LLM_API_KEY:+set (hidden)}"
...
uv run --project "$SCRIPT_DIR" atulya-embed daemon stop 2>/dev/null || true
sleep 1
...
OUTPUT=$(uv run --project "$SCRIPT_DIR" atulya-embed memory retain "$BANK_ID" "The user's favorite color is blue" 2>&1)
EXIT_CODE=$?
set -e
...
if [ $EXIT_CODE -ne 0 ]; then
    ...
    if [ -f ~/.atulya/daemon.log ]; then
        echo "=== daemon.log ==="
        tail -50 ~/.atulya/daemon.log
    else
        echo "No daemon.log found"
    fi
    if [ -f ~/.atulya/daemon.stderr ]; then
        echo ""
        echo "=== daemon.stderr ==="
        cat ~/.atulya/daemon.stderr
    else
        echo "No daemon.stderr found"
    fi
    echo ""
    echo "Checking for atulya-api..."
    which atulya-api 2>/dev/null || echo "atulya-api not in PATH"
...
START_TIME=$(python3 -c "import time; print(time.time())")
set +e
OUTPUT=$(uv run --project "$SCRIPT_DIR" atulya-embed memory recall "$BANK_ID" "What is the user's favorite color?" 2>&1)
EXIT_CODE=$?
set -e
END_TIME=$(python3 -c "import time; print(time.time())")
DURATION=$(python3 -c "print(f'{$END_TIME - $START_TIME:.2f}')")
...
START_TIME=$(python3 -c "import time; print(time.time())")
set +e
OUTPUT=$(uv run --project "$SCRIPT_DIR" atulya-embed memory retain "$BANK_ID" "User prefers Python over JavaScript" --context work 2>&1)
EXIT_CODE=$?
set -e
END_TIME=$(python3 -c "import time; print(time.time())")
DURATION=$(python3 -c "print(f'{$END_TIME - $START_TIME:.2f}')")
...
START_TIME=$(python3 -c "import time; print(time.time())")
set +e
JSON_OUTPUT=$(uv run --project "$SCRIPT_DIR" atulya-embed memory recall "$BANK_ID" "programming preferences" -o json 2>&1)
EXIT_CODE=$?
set -e
END_TIME=$(python3 -c "import time; print(time.time())")
DURATION=$(python3 -c "print(f'{$END_TIME - $START_TIME:.2f}')")
...
STATUS_OUTPUT=$(uv run --project "$SCRIPT_DIR" atulya-embed daemon status 2>&1)
echo "$STATUS_OUTPUT"
