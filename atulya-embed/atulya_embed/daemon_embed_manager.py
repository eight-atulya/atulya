...
class DaemonEmbedManager(EmbedManager):
    ...
    def get_database_url(self, profile: str, db_url: Optional[str] = None) -> str:
        ...
        return f"pg0://atulya-embed-{safe_profile}"
    ...
    def _find_api_command(self) -> list[str]:
        """Find the command to run atulya-api."""
        # Check if we're in development mode
        dev_api_path = Path(__file__).parent.parent.parent / "atulya-api"
        if dev_api_path.exists() and (dev_api_path / "pyproject.toml").exists():
            return ["uv", "run", "--project", str(dev_api_path), "atulya-api"]

        # Fall back to uvx for installed version
        from . import __version__

        api_version = os.getenv("ATULYA_EMBED_API_VERSION", __version__)
        return ["uvx", f"atulya-api@{api_version}"]
    ...
    def _start_daemon(self, config: dict, profile: str) -> bool:
        ...
        # Support both formats: simple keys ("llm_api_key") and env var format ("ATULYA_API_LLM_API_KEY")
        env = os.environ.copy()

        # Map of simple key -> env var key
        key_mapping = {
            "llm_api_key": "ATULYA_API_LLM_API_KEY",
            "llm_provider": "ATULYA_API_LLM_PROVIDER",
            "llm_model": "ATULYA_API_LLM_MODEL",
            "llm_base_url": "ATULYA_API_LLM_BASE_URL",
            "log_level": "ATULYA_API_LOG_LEVEL",
            "idle_timeout": "ATULYA_EMBED_DAEMON_IDLE_TIMEOUT",
        }
        ...
        db_override = config.get("ATULYA_EMBED_API_DATABASE_URL") or env.get("ATULYA_EMBED_API_DATABASE_URL")
        if db_override:
            env["ATULYA_API_DATABASE_URL"] = db_override
        else:
            env["ATULYA_API_DATABASE_URL"] = self.get_database_url(profile)

        database_url = env["ATULYA_API_DATABASE_URL"]
        is_pg0 = database_url.startswith("pg0://")

        # Set defaults if not provided
        if "ATULYA_API_LOG_LEVEL" not in env:
            env["ATULYA_API_LOG_LEVEL"] = "info"
        if "ATULYA_EMBED_DAEMON_IDLE_TIMEOUT" not in env:
            env["ATULYA_EMBED_DAEMON_IDLE_TIMEOUT"] = str(DEFAULT_DAEMON_IDLE_TIMEOUT)

        # On macOS, force CPU for embeddings/reranker to avoid MPS issues
        import platform

        if platform.system() == "Darwin":
            if "ATULYA_API_EMBEDDINGS_LOCAL_FORCE_CPU" not in env:
                env["ATULYA_API_EMBEDDINGS_LOCAL_FORCE_CPU"] = "1"
            if "ATULYA_API_RERANKER_LOCAL_FORCE_CPU" not in env:
                env["ATULYA_API_RERANKER_LOCAL_FORCE_CPU"] = "1"

        # Get idle timeout from env
        idle_timeout = int(env.get("ATULYA_EMBED_DAEMON_IDLE_TIMEOUT", str(DEFAULT_DAEMON_IDLE_TIMEOUT)))

        # Create log directory
        daemon_log.parent.mkdir(parents=True, exist_ok=True)
        env["ATULYA_API_DAEMON_LOG"] = str(daemon_log)
        ...
        try:
            ...
            error_msg = (
                ...
                "Install atulya-api with: pip install atulya-api"
            )
            error_panel = Panel(
    ...
    def ensure_running(self, config: dict, profile: str) -> bool:
        ...
        Args:
            config: Environment configuration dict (ATULYA_API_* vars)
