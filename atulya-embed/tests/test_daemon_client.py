...
from atulya_embed import daemon_client
...
class TestRunCli:
    ...
    def test_run_cli_with_external_api_url(self, config, mock_cli_binary, monkeypatch):
        """Test that external ATULYA_EMBED_API_URL skips daemon startup."""
        # Set up environment with external API URL
        external_api_url = "http://external-api:8000"
        monkeypatch.setenv("ATULYA_EMBED_API_URL", external_api_url)
        ...
        with (
            ...
            assert call_args.kwargs["env"]["ATULYA_API_URL"] == external_api_url
    ...
    def test_run_cli_without_external_api_url(self, config, mock_cli_binary, monkeypatch):
        ...
        # Ensure ATULYA_EMBED_API_URL is not set
        monkeypatch.delenv("ATULYA_EMBED_API_URL", raising=False)
        ...
        with (
            ...
            assert call_args.kwargs["env"]["ATULYA_API_URL"] == daemon_client.get_daemon_url()
    ...
    def test_run_cli_with_api_token(self, config, mock_cli_binary, monkeypatch):
        """Test that ATULYA_EMBED_API_TOKEN is passed through to the CLI."""
        # Set up environment with external API URL and token
        external_api_url = "http://external-api:8000"
        api_token = "test-bearer-token-12345"
        monkeypatch.setenv("ATULYA_EMBED_API_URL", external_api_url)
        monkeypatch.setenv("ATULYA_EMBED_API_TOKEN", api_token)
        ...
        with (
            ...
            assert call_args.kwargs["env"]["ATULYA_API_URL"] == external_api_url
            assert call_args.kwargs["env"]["ATULYA_API_KEY"] == api_token
