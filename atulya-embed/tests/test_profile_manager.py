...
from atulya_embed.profile_manager import (
    ProfileInfo,
    ProfileManager,
    ProfilePaths,
...
@pytest.fixture
def temp_atulya_dir(tmp_path, monkeypatch):
    """Create a temporary atulya directory for tests.
    ...
    from atulya_embed.cli import set_cli_profile_override

    set_cli_profile_override(None)

    # Clear ATULYA_EMBED_PROFILE env var
    monkeypatch.delenv("ATULYA_EMBED_PROFILE", raising=False)

    temp_home = tmp_path / "home"
    temp_home.mkdir()
    monkeypatch.setenv("HOME", str(temp_home))

    temp_config = temp_home / ".atulya"
    temp_config.mkdir()
    return temp_config


@pytest.fixture
def profile_manager(temp_atulya_dir):
...
class TestProfileManager:
    ...
    def test_create_profile_success(self, profile_manager, temp_atulya_dir):
        ...
        config_path = temp_atulya_dir / "profiles" / "test-profile.env"
        ...
        assert "ATULYA_EMBED_LLM_MODEL=gpt-4o-mini" in config_content

        # Verify metadata was created
        metadata_path = temp_atulya_dir / "profiles" / "metadata.json"
    ...
    def test_delete_profile(self, profile_manager, temp_atulya_dir):
        ...
        profiles_dir = temp_atulya_dir / "profiles"
    ...
    def test_set_active_profile(self, profile_manager, temp_atulya_dir):
        ...
        active_file = temp_atulya_dir / "active_profile"
    ...
    def test_clear_active_profile(self, profile_manager, temp_atulya_dir):
    ...
    def test_list_profiles(self, profile_manager, temp_atulya_dir):
    ...
    def test_list_profiles_with_active(self, profile_manager, temp_atulya_dir):
    ...
    def test_resolve_profile_paths_default(self, profile_manager, temp_atulya_dir):
        """Test resolving paths for default profile."""
        paths = profile_manager.resolve_profile_paths("")

        assert paths.config == temp_atulya_dir / "embed"
        assert paths.lock == temp_atulya_dir / "daemon.lock"
        assert paths.log == temp_atulya_dir / "daemon.log"
        assert paths.port == 8888

    def test_resolve_profile_paths_named(self, profile_manager, temp_atulya_dir):
        ...
        assert paths.config == temp_atulya_dir / "profiles" / "test-profile.env"
        assert paths.lock == temp_atulya_dir / "profiles" / "test-profile.lock"
        assert paths.log == temp_atulya_dir / "profiles" / "test-profile.log"
    ...
    def test_update_existing_profile(self, profile_manager, temp_atulya_dir):
        ...
        config_path = temp_atulya_dir / "profiles" / "test.env"
        config_content = config_path.read_text()
        assert "KEY=new_value" in config_content
        assert "KEY=old_value" not in config_content

    def test_metadata_persistence(self, profile_manager, temp_atulya_dir):
    ...
    def test_delete_active_profile_clears_active_file(self, profile_manager, temp_atulya_dir):
...
class TestValidateProfileExists:
    ...
    def test_validate_nonexistent_profile_fails(self, profile_manager, capsys):
        ...
        assert "atulya-embed configure --profile nonexistent" in captured.err
