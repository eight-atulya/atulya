...
def test_profile_config_is_loaded_for_daemon(temp_home):
    ...
    using only os.environ or hardcoded defaults. This caused ATULYA_EMBED_DAEMON_IDLE_TIMEOUT
    and other profile-specific settings to be silently ignored.
    """
    # Create a profile with custom configuration
    profile_dir = temp_home / ".atulya" / "profiles"
    ...
    profile_env_path.write_text(
        ...
        "ATULYA_API_LLM_API_KEY=sk-test-fake-key\n"
        "ATULYA_API_LLM_MODEL=gpt-4o-mini\n"
        "ATULYA_EMBED_DAEMON_IDLE_TIMEOUT=0\n"
        "ATULYA_API_LOG_LEVEL=debug\n"
    )
    ...
    from atulya_embed.profile_manager import ProfileManager
    ...
    assert profile_config["ATULYA_API_LLM_PROVIDER"] == "openai"
    assert profile_config["ATULYA_API_LLM_API_KEY"] == "sk-test-fake-key"
    assert profile_config["ATULYA_API_LLM_MODEL"] == "gpt-4o-mini"
    assert profile_config["ATULYA_EMBED_DAEMON_IDLE_TIMEOUT"] == "0"
    assert profile_config["ATULYA_API_LOG_LEVEL"] == "debug"
...
def test_load_config_file_uses_correct_profile(temp_home, monkeypatch):
    ...
    # Must clear ATULYA_EMBED_PROFILE env var to ensure test isolation
    monkeypatch.delenv("ATULYA_EMBED_PROFILE", raising=False)

    from atulya_embed.cli import load_config_file
    from atulya_embed.profile_manager import ProfileManager

    # Create default profile with one provider
    default_config_dir = temp_home / ".atulya"
    ...
    profile_name = "myapp"
    profile_env_path = profile_dir / f"{profile_name}.env"
    profile_env_path.write_text("ATULYA_API_LLM_PROVIDER=groq\n" "ATULYA_API_LLM_MODEL=llama-3.1-70b\n")
    ...
    monkeypatch.delenv("ATULYA_API_LLM_PROVIDER", raising=False)
    monkeypatch.delenv("ATULYA_API_LLM_MODEL", raising=False)

    # Test 1: Load default profile (no profile specified)
    set_cli_profile_override(None)
    load_config_file()

    assert os.environ.get("ATULYA_API_LLM_PROVIDER") == "openai"
    assert os.environ.get("ATULYA_API_LLM_MODEL") == "gpt-4o-mini"

    # Clear env
    monkeypatch.delenv("ATULYA_API_LLM_PROVIDER")
    monkeypatch.delenv("ATULYA_API_LLM_MODEL")
    ...
    assert os.environ.get("ATULYA_API_LLM_PROVIDER") == "groq", "Should load from named profile, not default"
    assert os.environ.get("ATULYA_API_LLM_MODEL") == "llama-3.1-70b", "Should load from named profile, not default"
...
def test_get_config_respects_profile(temp_home, monkeypatch):
    ...
    from atulya_embed.cli import get_config, set_cli_profile_override
    from atulya_embed.profile_manager import ProfileManager
    ...
    (default_config_dir / "embed").write_text(
        "ATULYA_API_LLM_PROVIDER=openai\n"
        "ATULYA_API_LLM_MODEL=gpt-4o-mini\n"
        "ATULYA_API_LLM_API_KEY=sk-default-key\n"
        "ATULYA_EMBED_BANK_ID=default-bank\n"
    )
    ...
    profile_env_path.write_text(
        ...
        "ATULYA_API_LLM_API_KEY=sk-ant-production\n"
        "ATULYA_EMBED_BANK_ID=production-bank\n"
    )
    ...
    monkeypatch.delenv("ATULYA_EMBED_BANK_ID", raising=False)
