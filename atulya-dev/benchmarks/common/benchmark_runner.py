...
def get_model_config() -> Dict[str, Dict[str, str]]:
    ...
    Returns:
        Dict with 'atulya', 'answer_generation', and 'judge' keys,
        each containing 'provider' and 'model' info.
    """
    # Memory/Atulya config (base config)
    memory_provider = os.getenv("ATULYA_API_LLM_PROVIDER", "groq")
    memory_model = os.getenv("ATULYA_API_LLM_MODEL", "openai/gpt-oss-120b")

    # Answer generation config (falls back to memory config)
    answer_provider = os.getenv("ATULYA_API_ANSWER_LLM_PROVIDER", memory_provider)
    answer_model = os.getenv("ATULYA_API_ANSWER_LLM_MODEL", memory_model)

    # Judge config (falls back to memory config)
    judge_provider = os.getenv("ATULYA_API_JUDGE_LLM_PROVIDER", memory_provider)
    judge_model = os.getenv("ATULYA_API_JUDGE_LLM_MODEL", memory_model)
...
def print_model_config():
    ...
    console.print(f"  Atulya:         {config['atulya']['provider']}/{config['atulya']['model']}")
    console.print(
...
async def create_memory_engine() -> MemoryEngine:
    ...
    - ATULYA_API_DATABASE_URL (default: "pg0")
    - ATULYA_API_LLM_PROVIDER (default: "groq")
    - ATULYA_API_LLM_API_KEY
    - ATULYA_API_LLM_MODEL (default: "openai/gpt-oss-120b")
    - ATULYA_API_LLM_BASE_URL (optional)

    Returns:
        Initialized MemoryEngine instance
    """
    memory = MemoryEngine(
        db_url=os.getenv("ATULYA_API_DATABASE_URL", "pg0"),
        ...
        memory_llm_base_url=os.getenv("ATULYA_API_LLM_BASE_URL") or None,  # Use None to get provider defaults
    )
...
class BenchmarkRunner:
    ...
    def __init__(
        ...
        self.memory = memory or MemoryEngine(
            ...
            memory_llm_provider=os.getenv("ATULYA_API_LLM_PROVIDER", "groq"),
            memory_llm_api_key=os.getenv("ATULYA_API_LLM_API_KEY"),
            memory_llm_model=os.getenv("ATULYA_API_LLM_MODEL", "openai/gpt-oss-20b"),
