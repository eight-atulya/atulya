...
async def run_benchmark(
    ...
    if api_url:
        from benchmarks.common.benchmark_runner import AtulyaClientAdapter

        memory = AtulyaClientAdapter(base_url=api_url)
...
def generate_markdown_table(results: dict, use_reflect: bool = False):
    ...
    if "model_config" in results:
        ...
        lines.append("")
        lines.append(f"- **Atulya**: {config['atulya']['provider']}/{config['atulya']['model']}")
        lines.append(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Atulya API URL (default: use local memory, example: http://localhost:8888)",
    )
    parser.add_argument(
