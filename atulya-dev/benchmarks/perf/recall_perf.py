...
Usage (run from atulya-api/):
    cd atulya-api

    # Generate a small bank (~10K memory units):
    uv run python ../atulya-dev/benchmarks/perf/recall_perf.py generate \\
        --bank-id recall-perf-small2 --scale small

    # Benchmark recall:
    uv run python ../atulya-dev/benchmarks/perf/recall_perf.py benchmark \\
        --bank-id recall-perf-small --query "database migration" --iterations 5

    # Clean up:
    uv run python ../atulya-dev/benchmarks/perf/recall_perf.py clean \\
...
def _build_engine(*, disable_observations: bool = False) -> "Any":
    ...
    if disable_observations:
        os.environ["ATULYA_API_ENABLE_OBSERVATIONS"] = "false"
    engine = MemoryEngine(
        db_url=db_url,
...
class _RRFReranker:
    ...
    async def rerank(self, query: str, candidates: list) -> list:
        from atulya_api.engine.search.types import ScoredResult
