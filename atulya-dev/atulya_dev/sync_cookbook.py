#!/usr/bin/env python3
"""
Syncs content from the atulya-cookbook repository.
...
Usage: sync-cookbook (after installing atulya-dev)
...
COOKBOOK_REPO = "https://github.com/eight-atulya/atulya-cookbook.git"
IGNORE_DIRS = {".git", "notebooks", "node_modules", "__pycache__", ".venv", "venv"}


def get_docs_dir() -> Path:
    """Find the atulya-docs src/pages/cookbook directory relative to this script."""
    script_dir = Path(__file__).parent
    return script_dir.parent.parent / "atulya-docs" / "src" / "pages" / "cookbook"
...
def extract_tags_from_readme(readme_path: Path) -> dict[str, str]:
    ...
    - Object literal: tags: { sdk: "atulya-client", topic: "Learning" }
...
def process_notebooks(cookbook_dir: Path, recipes_dir: Path) -> list[dict]:
    ...
    for i, notebook_path in enumerate(files):
        ...
        notebook_url = f"https://github.com/eight-atulya/atulya-cookbook/blob/main/notebooks/{notebook_path.name}"
...
def process_applications(cookbook_dir: Path, apps_dir: Path) -> list[dict]:
    ...
    for entry in sorted(applications_dir.iterdir()):
        ...
        app_url = f"https://github.com/eight-atulya/atulya-cookbook/tree/main/applications/{entry.name}"
