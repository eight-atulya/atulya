...
Manages running different git versions of the Atulya API for upgrade testing.
...
class VersionRunner:
    """
    Manages running a specific git version of the Atulya API.
    ...
    def __init__(
        ...
        self.db_url = db_url
        self.port = port
        self.llm_provider = llm_provider or os.getenv("ATULYA_API_LLM_PROVIDER", "groq")
        self.llm_api_key = llm_api_key or os.getenv("ATULYA_API_LLM_API_KEY") or os.getenv("GROQ_API_KEY")
        self.llm_model = llm_model or os.getenv("ATULYA_API_LLM_MODEL", "llama-3.3-70b-versatile")
    ...
    def setup(self) -> None:
        ...
        self._temp_dir = tempfile.mkdtemp(prefix=f"atulya-{self.version}-")
        ...
        api_path = self.work_dir / "atulya-api"
        logger.info(f"Installing atulya-api from {api_path}")
    ...
    def _get_venv_path(self) -> Path:
        ...
        if self._is_current:
            ...
            api_venv = self.work_dir / "atulya-api" / ".venv"

            if (workspace_venv / "bin" / "atulya-api").exists():
                return workspace_venv
            elif (api_venv / "bin" / "atulya-api").exists():
                return api_venv
            else:
    ...
    def start(self) -> ServerInfo:
        ...
        atulya_api_bin = venv_path / "bin" / "atulya-api"

        if not atulya_api_bin.exists():
            raise RuntimeError(f"atulya-api binary not found at {atulya_api_bin}")

        env = os.environ.copy()
        env.update(
            {
                "ATULYA_API_PORT": str(self.port),
                "ATULYA_API_DATABASE_URL": self.db_url,
                "ATULYA_API_HOST": "127.0.0.1",
                "ATULYA_API_LLM_PROVIDER": self.llm_provider,
                "ATULYA_API_LLM_API_KEY": self.llm_api_key or "",
                "ATULYA_API_LLM_MODEL": self.llm_model,
                "PYTHONUNBUFFERED": "1",
            }
        )
        ...
        # (atulya-api loads .env with override=True which would override our env vars)
        if self._is_current:
            # Create a temp directory for HEAD to avoid workspace .env
            self._head_cwd = tempfile.mkdtemp(prefix="atulya-head-cwd-")
            cwd = self._head_cwd
        else:
