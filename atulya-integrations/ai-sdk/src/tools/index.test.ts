...
import { createAtulyaTools, type AtulyaClient } from './index.js';

describe('createAtulyaTools', () => {
  let mockClient: AtulyaClient;
  ...
  describe('tool creation', () => {
    it('should create all tools', () => {
      const tools = createAtulyaTools({ client: mockClient, bankId: 'test-bank' });
  ...
  describe('budget defaults', () => {
    ...
    it('should accept low/mid/high budget values', async () => {
      ...
      for (const budget of ['low', 'mid', 'high'] as const) {
        const tools = createAtulyaTools({ client: mockClient, bankId: 'test-bank', recall: { budget } });
        await tools.recall.execute({ query: 'Test' });
  ...
  describe('bankId enforcement', () => {
    it('should always use the bankId from constructor options', async () => {
      const tools = createAtulyaTools({ client: mockClient, bankId: 'forced-bank' });
      vi.mocked(mockClient.retain).mockResolvedValue({
        success: true,
        bank_id: 'forced-bank',
        items_count: 1,
