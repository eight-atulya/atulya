...
CrewAI runs inside an async event loop. The Atulya client's sync
...
event loop for all Atulya API calls, ensuring the aiohttp session
...
def call_sync(fn: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    """Call a sync Atulya client method safely.
