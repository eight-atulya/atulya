"""Unit tests for AtulyaReflectTool."""

from unittest.mock import MagicMock, patch

from atulya_crewai import AtulyaReflectTool, configure, reset_config
...
class TestReflectTool:
    ...
    @patch("atulya_crewai.tools.call_sync", side_effect=_passthrough)
    def test_reflect_returns_text(self, _mock_cs):
        tool = AtulyaReflectTool(bank_id="test-bank")
    ...
    def test_reflect_passes_budget(self, _mock_cs):
        tool = AtulyaReflectTool(bank_id="test-bank", budget="high")
        mock_client = MagicMock()
        mock_result = MagicMock()
    ...
    @patch("atulya_crewai.tools.call_sync", side_effect=_passthrough)
    def test_reflect_raises_atulya_error_on_failure(self, _mock_cs):
        tool = AtulyaReflectTool(bank_id="test-bank")
        mock_client = MagicMock()
    ...
    def test_tool_metadata(self):
        tool = AtulyaReflectTool(bank_id="test-bank")
        assert tool.name == "atulya_reflect"
