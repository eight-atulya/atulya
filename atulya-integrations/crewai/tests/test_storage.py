"""Unit tests for AtulyaStorage."""

from unittest.mock import MagicMock, patch

from atulya_crewai import AtulyaStorage, configure, reset_config
from atulya_crewai.errors import AtulyaError
...
class TestAtulyaStorage:
    ...
    def _make_storage(self, **kwargs):
        ...
        storage = AtulyaStorage(bank_id="test-bank", **kwargs)
    ...
    @patch("atulya_crewai.storage.call_sync", side_effect=_passthrough)
    ...
    @patch("atulya_crewai.storage.call_sync", side_effect=_passthrough)
    def test_save_raises_atulya_error_on_failure(self, _mock_cs):
        storage, mock_client = self._make_storage()
        mock_client.retain.side_effect = RuntimeError("connection refused")

        with pytest.raises(AtulyaError, match="Failed to store memory"):
            storage.save("text")
    ...
    @patch("atulya_crewai.storage.call_sync", side_effect=_passthrough)
    def test_search_raises_atulya_error_on_failure(self, _mock_cs):
        storage, mock_client = self._make_storage()
        mock_client.recall.side_effect = RuntimeError("timeout")

        with pytest.raises(AtulyaError, match="Failed to search memories"):
            storage.search("test")
    ...
    def test_per_agent_banks_resolves_bank_id(self):
        storage = AtulyaStorage(bank_id="crew", per_agent_banks=True)
    ...
    def test_custom_bank_resolver(self):
        ...
        storage = AtulyaStorage(bank_id="crew", bank_resolver=resolver)
