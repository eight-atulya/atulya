"""CrewAI Storage backend powered by Atulya.

Implements CrewAI's Storage interface (save/search/reset) using
Atulya's retain/recall APIs for persistent agent memory.
"""
...
class AtulyaStorage(Storage):
    """CrewAI Storage backend that persists memories to Atulya.

    Maps CrewAI's storage interface to Atulya's memory API:
    ...
    Args:
        bank_id: The Atulya memory bank ID for this crew.
    ...
    def __init__(
        ...
        config = get_config()
        self._api_url = atulya_api_url or (
            config.atulya_api_url if config else "http://localhost:8888"
        )
    ...
    def _get_client(self) -> Any:
        """Get or create a thread-local Atulya client.
    ...
    def save(
        ...
        Raises:
            AtulyaError: If the retain operation fails.
        """
        bank_id = self._resolve_bank_id(agent)
        self._ensure_bank(bank_id)

        # Build retain metadata — Atulya requires dict[str, str]
        ...
        try:
            ...
            raise AtulyaError(f"Failed to store memory: {e}") from e
    ...
    def search(
        ...
        """Search memories in Atulya via recall.
        ...
        Raises:
            AtulyaError: If the recall operation fails.
        """
        ...
        try:
            ...
            for i, r in enumerate(recall_results[:limit]):
                # Atulya returns results ordered by relevance.
            ...
            raise AtulyaError(f"Failed to search memories: {e}") from e
