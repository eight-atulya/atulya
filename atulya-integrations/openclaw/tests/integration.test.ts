/**
 * Integration tests for the Atulya OpenClaw integration.
 *
 * Tests both HTTP mode (direct API calls) and Embed mode (subprocess/daemon).
 *
 * Requirements:
 *   HTTP mode:  Running Atulya API at ATULYA_API_URL (default: http://localhost:8888)
 *   Embed mode: atulya-embed package at ATULYA_EMBED_PACKAGE_PATH
 *               + LLM credentials (ATULYA_API_LLM_PROVIDER / ATULYA_API_LLM_API_KEY)
 *
 * Run:
...
import { AtulyaClient } from '../src/client.js';
import { AtulyaEmbedManager } from '../src/embed-manager.js';
...
const LLM_PROVIDER = process.env.ATULYA_API_LLM_PROVIDER || '';
const LLM_API_KEY = process.env.ATULYA_API_LLM_API_KEY || '';
const LLM_MODEL = process.env.ATULYA_API_LLM_MODEL || '';

// Embed package path – defaults to the sibling atulya-embed directory in the repo
const EMBED_PACKAGE_PATH =
  process.env.ATULYA_EMBED_PACKAGE_PATH ||
  join(__dirname, '..', '..', '..', 'atulya-embed');
...
describe('AtulyaClient – HTTP Mode', () => {
  let client: AtulyaClient;

  beforeAll(async () => {
    const reachable = await waitForApi(ATULYA_API_URL);
    if (!reachable) {
      throw new Error(
        `Atulya API not reachable at ${ATULYA_API_URL}. ` +
          'Start the server before running integration tests.',
      );
    }

    client = new AtulyaClient({
      llmProvider: LLM_PROVIDER || 'openai',
      llmApiKey: LLM_API_KEY || 'test-key',
      llmModel: LLM_MODEL || undefined,
      apiUrl: ATULYA_API_URL,
    });
  });
...
describe('AtulyaClient – Embed Mode (Subprocess)', () => {
  let client: AtulyaClient;
  let embedManager: AtulyaEmbedManager;
    ...
    if (!hasEmbedCredentials) {
      console.warn(
        '[Integration] Skipping embed mode tests: ' +
          'ATULYA_API_LLM_PROVIDER and ATULYA_API_LLM_API_KEY must both be set.',
      );
      return;
    }
