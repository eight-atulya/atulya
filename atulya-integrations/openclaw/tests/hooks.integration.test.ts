 ...
 *
 * Requirements:
 *   Running Atulya API at ATULYA_API_URL (default: http://localhost:8888)
 *
 * Run:
...
function createMockApi(pluginConfig: Partial<PluginConfig> = {}): MockApiHandle {
  ...
  const api: MoltbotPluginAPI = {
    config: {
      plugins: {
        entries: {
          'atulya-openclaw': { enabled: true, config: pluginConfig as PluginConfig },
        },
      },
    },
...
let recallSpy: ReturnType<typeof vi.spyOn<AtulyaClient, 'recall'>>;
let retainSpy: ReturnType<typeof vi.spyOn<AtulyaClient, 'retain'>>;

beforeAll(async () => {
  apiReachable = await waitForApi(ATULYA_API_URL, 8000);
  if (!apiReachable) {
    console.warn(
      `[Hooks Integration] Atulya API not reachable at ${ATULYA_API_URL} – skipping hook tests.`,
    );
    return;
  }
  ...
  process.env.ATULYA_API_LLM_PROVIDER = 'openai';
  process.env.ATULYA_API_LLM_API_KEY = 'test-key-hooks';
  // Point the plugin at the running test API.
  process.env.ATULYA_EMBED_API_URL = ATULYA_API_URL;

  const mod = await import('../src/index.js');
  const { AtulyaClient } = await import('../src/client.js');
  ...
  recallSpy = vi.spyOn(AtulyaClient.prototype, 'recall') as ReturnType<typeof vi.spyOn<AtulyaClient, 'recall'>>;
  retainSpy = vi.spyOn(AtulyaClient.prototype, 'retain') as ReturnType<typeof vi.spyOn<AtulyaClient, 'retain'>>;
}, 30_000);

afterAll(async () => {
  vi.restoreAllMocks();
  delete process.env.ATULYA_API_LLM_PROVIDER;
  delete process.env.ATULYA_API_LLM_API_KEY;
  delete process.env.ATULYA_EMBED_API_URL;
...
describe('before_prompt_build hook', () => {
  ...
  it('returns { prependContext } with <atulya_memories> when recall returns results', async () => {
    if (!apiReachable) return;
    ...
    expect(result.prependContext).toContain('<atulya_memories>');
...
describe('agent_end hook', () => {
  ...
  it('strips <atulya_memories> tags from content before retaining', async () => {
    if (!apiReachable) return;
    retainSpy.mockResolvedValue(OK_RETAIN);
    ...
    expect(retainSpy).toHaveBeenCalledOnce();
    const [req] = retainSpy.mock.calls[0];
    expect(req.content).not.toContain('<atulya_memories>');
    expect(req.content).not.toContain('</atulya_memories>');
