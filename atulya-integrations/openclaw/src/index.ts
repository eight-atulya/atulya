...
import { AtulyaEmbedManager } from './embed-manager.js';
import { AtulyaClient, type AtulyaClientOptions } from './client.js';
...
// Module-level state
let embedManager: AtulyaEmbedManager | null = null;
let client: AtulyaClient | null = null;
let clientOptions: AtulyaClientOptions | null = null;
...
const clientsByBankId = new Map<string, AtulyaClient>();
...
async function lazyReinit(): Promise<void> {
  ...
  debug('[Atulya] Attempting lazy re-initialization...');
  try {
    ...
    process.env.ATULYA_EMBED_API_URL = externalApi.apiUrl;
    if (externalApi.apiToken) {
      process.env.ATULYA_EMBED_API_TOKEN = externalApi.apiToken;
    }
    ...
    client = new AtulyaClient(clientOptions);
    ...
    debug('[Atulya] ✓ Lazy re-initialization succeeded');
  } catch (error) {
    console.warn(`[Atulya] Lazy re-initialization failed (will retry in ${REINIT_COOLDOWN_MS / 1000}s):`, error instanceof Error ? error.message : error);
  } finally {
    isReinitInProgress = false;
  }
}
  ...
  (global as any).__atulyaClient = {
    ...
    waitForReady: async () => {
      ...
      if (initPromise) {
        try {
          ...
          // Attempt lazy re-initialization so Atulya recovers
    ...
    getClientForContext: async (ctx: PluginHookAgentContext | undefined) => {
      ...
      if (!bankClient) {
        ...
        bankClient = new AtulyaClient(clientOptions);
      ...
      if (config.bankMission && config.dynamicBankId && !banksWithMissionSet.has(bankId)) {
        try {
          ...
          debug(`[Atulya] Set mission for new bank: ${bankId}`);
        } catch (error) {
          // Log but don't fail - bank mission is not critical
          console.warn(`[Atulya] Could not set bank mission for ${bankId}: ${error}`);
        }
      }
...
export function stripMemoryTags(content: string): string {
  content = content.replace(/<atulya_memories>[\s\S]*?<\/atulya_memories>/g, '');
...
export function deriveBankId(ctx: PluginHookAgentContext | undefined, pluginConfig: PluginConfig): string {
  ...
  for (const f of fields) {
    if (!validFields.has(f)) {
      console.warn(`[Atulya] Unknown dynamicBankGranularity field "${f}" — will resolve to "unknown" in bank ID. Valid fields: agent, channel, user, provider`);
    }
  }
  ...
  if (fields.includes('user') && ctx && !ctx.senderId) {
    debug('[Atulya] senderId not available in context — bank ID will use "anonymous". Ensure your OpenClaw provider passes senderId.');
  }
...
function detectLLMConfig(pluginConfig?: PluginConfig): {
  ...
  // Override values from ATULYA_API_LLM_* env vars (highest priority)
  const overrideProvider = process.env.ATULYA_API_LLM_PROVIDER;
  const overrideModel = process.env.ATULYA_API_LLM_MODEL;
  const overrideKey = process.env.ATULYA_API_LLM_API_KEY;
  const overrideBaseUrl = process.env.ATULYA_API_LLM_BASE_URL;
  ...
  if (overrideProvider) {
    ...
    if (!overrideKey && !noKeyRequired.includes(overrideProvider)) {
      throw new Error(
        `ATULYA_API_LLM_PROVIDER is set to "${overrideProvider}" but ATULYA_API_LLM_API_KEY is not set.\n` +
        `Please set: export ATULYA_API_LLM_API_KEY=your-api-key`
      );
    }
    ...
    return {
      ...
      source: 'ATULYA_API_LLM_PROVIDER override',
    };
  }
  ...
  for (const providerInfo of PROVIDER_DETECTION) {
    ...
    if (apiKey) {
      return {
        ...
        apiKey,
        model: overrideModel || providerInfo.defaultModel,
        baseUrl: overrideBaseUrl, // Only use explicit ATULYA_API_LLM_BASE_URL
  ...
  // Allow empty LLM config if using external Atulya API (server handles LLM)
  ...
  throw new Error(
    `No LLM configuration found for Atulya memory plugin.\n\n` +
...
function detectExternalApi(pluginConfig?: PluginConfig): {
  ...
  const apiUrl = process.env.ATULYA_EMBED_API_URL || pluginConfig?.atulyaApiUrl || null;
  const apiToken = process.env.ATULYA_EMBED_API_TOKEN || pluginConfig?.atulyaApiToken || null;
  return { apiUrl, apiToken };
}

/**
 * Build AtulyaClientOptions from LLM config, plugin config, and external API settings.
 */
...
/**
 * Health check for external Atulya API.
...
async function checkExternalApiHealth(apiUrl: string, apiToken?: string | null): Promise<void> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      debug(`[Atulya] Checking external API health at ${healthUrl}... (attempt ${attempt}/${maxRetries})`);
      ...
      debug(`[Atulya] External API health: ${JSON.stringify(data)}`);
      return;
    } catch (error) {
      if (attempt < maxRetries) {
        debug(`[Atulya] Health check attempt ${attempt} failed, retrying in ${retryDelay}ms...`);
        await new Promise(resolve => setTimeout(resolve, retryDelay));
      } else {
        throw new Error(`Cannot connect to external Atulya API at ${apiUrl}: ${error}`, { cause: error });
      }
    }
  }
}
...
function getPluginConfig(api: MoltbotPluginAPI): PluginConfig {
  const config = api.config.plugins?.entries?.['atulya-openclaw']?.config || {};
  ...
  return {
    ...
    atulyaApiUrl: config.atulyaApiUrl,
    atulyaApiToken: config.atulyaApiToken,
...
export default function (api: MoltbotPluginAPI) {
  try {
    ...
    debug('[Atulya] Detecting LLM config...');
    ...
    if (llmConfig.provider === 'ollama') {
      debug(`[Atulya] ✓ Using provider: ${llmConfig.provider}, model: ${modelInfo} (${llmConfig.source})`);
    } else {
      debug(`[Atulya] ✓ Using provider: ${llmConfig.provider}, model: ${modelInfo} (${llmConfig.source}${baseUrlInfo})`);
    }
    if (pluginConfig.bankMission) {
      debug(`[Atulya] Custom bank mission configured: "${pluginConfig.bankMission.substring(0, 50)}..."`);
    }
    ...
    if (pluginConfig.dynamicBankId) {
      ...
      debug(`[Atulya] ✓ Dynamic bank IDs enabled${prefixInfo} - each channel gets isolated memory`);
    } else {
      debug(`[Atulya] Dynamic bank IDs disabled - using static bank: ${DEFAULT_BANK_NAME}`);
    }
    ...
    if (externalApi.apiUrl) {
      ...
      usingExternalApi = true;
      debug(`[Atulya] ✓ Using external API: ${externalApi.apiUrl}`);

      // Set env vars so CLI commands (uvx atulya-embed) use external API
      process.env.ATULYA_EMBED_API_URL = externalApi.apiUrl;
      if (externalApi.apiToken) {
        process.env.ATULYA_EMBED_API_TOKEN = externalApi.apiToken;
        debug('[Atulya] API token configured');
      }
    } else {
      debug(`[Atulya] Daemon idle timeout: ${pluginConfig.daemonIdleTimeout}s (0 = never timeout)`);
      debug(`[Atulya] API Port: ${apiPort}`);
    }

    // Initialize in background (non-blocking)
    debug('[Atulya] Starting initialization in background...');
    initPromise = (async () => {
      try {
        if (usingExternalApi && externalApi.apiUrl) {
          // External API mode - check health, skip daemon startup
          debug('[Atulya] External API mode - skipping local daemon...');
          await checkExternalApiHealth(externalApi.apiUrl, externalApi.apiToken);

          // Initialize client with direct HTTP mode
          debug('[Atulya] Creating AtulyaClient (HTTP mode)...');
          ...
          debug(`[Atulya] Default bank: ${defaultBankId}`);
          client.setBankId(defaultBankId);
          ...
          if (pluginConfig.bankMission && !pluginConfig.dynamicBankId) {
            debug(`[Atulya] Setting bank mission...`);
            await client.setBankMission(pluginConfig.bankMission);
          }

          isInitialized = true;
          debug('[Atulya] ✓ Ready (external API mode)');
        } else {
          // Local daemon mode - start atulya-embed daemon
          debug('[Atulya] Creating AtulyaEmbedManager...');
          embedManager = new AtulyaEmbedManager(
            apiPort,
            llmConfig.provider || "",
            llmConfig.apiKey || "",
            llmConfig.model,
          ...
          debug('[Atulya] Starting embedded server...');
          await embedManager.start();

          // Initialize client (local daemon mode — no apiUrl)
          debug('[Atulya] Creating AtulyaClient (subprocess mode)...');
        ...
        console.error('[Atulya] Initialization error:', error);
        throw error;
      }
    })();
    ...
    debug('[Atulya] Registering service...');
    api.registerService({
      id: 'atulya-memory',
      async start() {
        debug('[Atulya] Service start called...');
        ...
        if (initPromise) {
          try {
            ...
            console.error('[Atulya] Initial initialization failed:', error);
        ...
        if (usingExternalApi) {
          ...
          if (externalApi.apiUrl && isInitialized) {
            try {
              await checkExternalApiHealth(externalApi.apiUrl, externalApi.apiToken);
              debug('[Atulya] External API is healthy');
              return;
            } catch (error) {
              console.error('[Atulya] External API health check failed:', error);
          ...
          if (embedManager && isInitialized) {
            ...
            if (healthy) {
              debug('[Atulya] Daemon is healthy');
              return;
            }

            debug('[Atulya] Daemon is not responding - reinitializing...');
        ...
        if (!isInitialized) {
          ...
          if (externalApi.apiUrl) {
            ...
            isInitialized = true;
            debug('[Atulya] Reinitialization complete (external API mode)');
          } else {
            // Local daemon mode
            embedManager = new AtulyaEmbedManager(
              apiPort,
            ...
            isInitialized = true;
            debug('[Atulya] Reinitialization complete');
          }
        }
      },
      ...
      async stop() {
        try {
          ...
          debug('[Atulya] Service stopped');
        } catch (error) {
          console.error('[Atulya] Service stop error:', error);
          throw error;
        }
      },
    });

    debug('[Atulya] Plugin loaded successfully');

    // Register agent hooks for auto-recall and auto-retention
    if (registeredApis.has(api)) {
      debug('[Atulya] Hooks already registered for this api instance, skipping duplicate registration');
      return;
    }
    registeredApis.add(api);
    debug('[Atulya] Registering agent hooks...');
    ...
    api.on('before_prompt_build', async (event: any, ctx?: PluginHookAgentContext) => {
      try {
        ...
        if (ctx?.messageProvider && pluginConfig.excludeProviders?.includes(ctx.messageProvider)) {
          debug(`[Atulya] Skipping recall for excluded provider: ${ctx.messageProvider}`);
          return;
        }

        // Skip auto-recall when disabled (agent has its own recall tool)
        if (!pluginConfig.autoRecall) {
          debug('[Atulya] Auto-recall disabled via config, skipping');
          return;
        }
        ...
        debug(`[Atulya] before_prompt_build - bank: ${bankId}, channel: ${ctx?.messageProvider}/${ctx?.channelId}`);
        debug(`[Atulya] event keys: ${Object.keys(event).join(', ')}`);
        debug(`[Atulya] event.context keys: ${Object.keys(event.context ?? {}).join(', ')}`);
        ...
        debug(`[Atulya] extractRecallQuery input lengths - raw: ${event.rawMessage?.length ?? 0}, prompt: ${event.prompt?.length ?? 0}`);
        const extracted = extractRecallQuery(event.rawMessage, event.prompt);
        if (!extracted) {
          debug('[Atulya] extractRecallQuery returned null, skipping recall');
          return;
        }
        debug(`[Atulya] extractRecallQuery result length: ${extracted.length}`);
        ...
        debug(`[Atulya] event.messages count: ${messageCount}, roles: ${sessionMessages.map((m: any) => m.role).join(',')}`);
        if (recallContextTurns > 1 && messageCount === 0) {
          debug('[Atulya] recallContextTurns > 1 but event.messages is empty — prior context unavailable at before_agent_start for this provider');
        }
        ...
        const clientGlobal = (global as any).__atulyaClient;
        if (!clientGlobal) {
          debug('[Atulya] Client global not available, skipping auto-recall');
          return;
        }
        ...
        if (!client) {
          debug('[Atulya] Client not initialized, skipping auto-recall');
          return;
        }

        debug(`[Atulya] Auto-recall for bank ${bankId}, full query:\n---\n${prompt}\n---`);
        ...
        if (existing) {
          debug(`[Atulya] Reusing in-flight recall for bank ${bankId}`);
...
${memoriesFormatted}
        ...
        debug(`[Atulya] Auto-recall: Injecting ${results.length} memories from bank ${bankId}`);

        // Inject context before the user message
        return { prependContext: contextMessage };
      } catch (error) {
        if (error instanceof DOMException && error.name === 'TimeoutError') {
          console.warn(`[Atulya] Auto-recall timed out after ${RECALL_TIMEOUT_MS}ms, skipping memory injection`);
        } else if (error instanceof Error && error.name === 'AbortError') {
          console.warn(`[Atulya] Auto-recall aborted after ${RECALL_TIMEOUT_MS}ms, skipping memory injection`);
        } else {
          console.error('[Atulya] Auto-recall error:', error);
        }
        return;
      }
    });
    ...
    api.on('agent_end', async (event: any, ctx?: PluginHookAgentContext) => {
      try {
        ...
        if (effectiveCtx?.messageProvider && pluginConfig.excludeProviders?.includes(effectiveCtx.messageProvider)) {
          debug(`[Atulya] Skipping retain for excluded provider: ${effectiveCtx.messageProvider}`);
          return;
        }
        ...
        debug(`[Atulya Hook] agent_end triggered - bank: ${bankId}`);

        if (event.success === false) {
          debug('[Atulya Hook] Agent run failed, skipping retention');
          return;
        }
        ...
        if (!Array.isArray(event.context?.sessionEntry?.messages ?? event.messages) || (event.context?.sessionEntry?.messages ?? event.messages ?? []).length === 0) {
          debug('[Atulya Hook] No messages in event, skipping retention');
          return;
        }

        if (pluginConfig.autoRetain === false) {
          debug('[Atulya Hook] autoRetain is disabled, skipping retention');
          return;
        }
        ...
        if (retainEveryN > 1) {
          ...
          if (turnCount % retainEveryN !== 0) {
            ...
            debug(`[Atulya Hook] Turn ${turnCount}/${retainEveryN}, skipping retain (next at turn ${nextRetainAt})`);
            return;
          }
          ...
          debug(`[Atulya Hook] Turn ${turnCount}: chunked retain firing (window: ${windowTurns} turns, ${messagesToRetain.length} messages)`);
        }

        const retention = prepareRetentionTranscript(messagesToRetain, pluginConfig, retainFullWindow);
        if (!retention) {
          debug('[Atulya Hook] No messages to retain (filtered/short/no-user)');
          return;
        }
        ...
        const clientGlobal = (global as any).__atulyaClient;
        if (!clientGlobal) {
          console.warn('[Atulya] Client global not found, skipping retain');
          return;
        }
        ...
        if (!client) {
          console.warn('[Atulya] Client not initialized, skipping retain');
          return;
        }
        ...
        // Retain to Atulya
        debug(`[Atulya] Retaining to bank ${bankId}, document: ${documentId}, chars: ${transcript.length}\n---\n${transcript.substring(0, 500)}${transcript.length > 500 ? '\n...(truncated)' : ''}\n---`);
        await client.retain({
          content: transcript,
        ...
        debug(`[Atulya] Retained ${messageCount} messages to bank ${bankId} for session ${documentId}`);
      } catch (error) {
        console.error('[Atulya] Error retaining messages:', error);
      }
    });
    debug('[Atulya] Hooks registered');
  } catch (error) {
    console.error('[Atulya] Plugin loading error:', error);
    if (error instanceof Error) {
      console.error('[Atulya] Error stack:', error.stack);
    }
    throw error;
  }
}
