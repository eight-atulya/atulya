...
describe('stripMemoryTags', () => {
  it('strips simple atulya_memories tags', () => {
    const input =
  ...
  it('strips multiple atulya_memories blocks', () => {
    const input =
      'Start\n<atulya_memories>\nBlock 1\n</atulya_memories>\nMiddle\n<atulya_memories>\nBlock 2\n</atulya_memories>\nEnd';
  ...
  it('strips tags from a real-world agent conversation with injected memories', () => {
    ...
    expect(result).not.toContain('<atulya_memories>');
    expect(result).not.toContain('</atulya_memories>');
...
describe('prepareRetentionTranscript', () => {
  ...
  it('strips memory tags from retained content (feedback loop prevention)', () => {
    const messages = [
      ...
      { role: 'assistant', content: '<atulya_memories>\nUser prefers dark mode\n</atulya_memories>\nHere is how to enable dark mode.' }
    ];
    const result = prepareRetentionTranscript(messages, baseConfig);
    expect(result).not.toBeNull();
    expect(result?.transcript).not.toContain('<atulya_memories>');
