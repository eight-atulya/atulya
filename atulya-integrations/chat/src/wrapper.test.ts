...
import { withAtulyaChat } from './wrapper.js';
import type {
  AtulyaClient,
  ChatThread,
  ChatMessage,
...
function mockClient(overrides?: Partial<AtulyaClient>): AtulyaClient {
  return {
...
describe('withAtulyaChat', () => {
  ...
  describe('auto-recall', () => {
    it('recalls by default', async () => {
      const handler = vi.fn();
      const wrapped = withAtulyaChat({ client, bankId: 'bank' }, handler);
      ...
      const ctx: AtulyaChatContext = handler.mock.calls[0][2];
    ...
    it('handles recall errors gracefully', async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        '[atulya-chat] Auto-recall failed:',
        expect.any(Error)
      );
  ...
  describe('auto-retain', () => {
    ...
    it('handles retain errors gracefully', async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        '[atulya-chat] Auto-retain failed:',
        expect.any(Error)
      );
      // Handler still runs
