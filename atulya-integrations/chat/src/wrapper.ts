...
/**
 * Wraps a Chat SDK handler to automatically provide Atulya memory context.
 *
 ...
 * 4. Builds a AtulyaChatContext and passes it to the handler
 *
 ...
 * ```
 */
export function withAtulyaChat<TState = unknown>(
  options: AtulyaChatOptions,
  handler: AtulyaChatHandler<TState>
  ...
  return async (thread: ChatThread<TState>, message: ChatMessage): Promise<void> => {
    ...
    if (retainEnabled && message.text && !message.author.isMe) {
      const retainPromise = client
        ...
        .catch((err) => {
          console.warn('[atulya-chat] Auto-retain failed:', err);
        });
    ...
    if (recallEnabled && message.text) {
      try {
        ...
        console.warn('[atulya-chat] Auto-recall failed:', err);
      }
    }

    // 4. Build context
    const ctx: AtulyaChatContext = {
