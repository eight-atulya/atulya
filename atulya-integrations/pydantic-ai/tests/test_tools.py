"""Unit tests for Atulya Pydantic AI tools."""

from unittest.mock import AsyncMock, MagicMock, patch

import pytest

from atulya_pydantic_ai import (
    configure,
    create_atulya_tools,
    memory_instructions,
    reset_config,
)
from atulya_pydantic_ai.errors import AtulyaError


def _mock_client():
    """Create a mock Atulya client with async methods."""
    client = MagicMock()
...
class TestCreateAtulyaTools:
    ...
    def test_returns_three_tools_by_default(self):
        client = _mock_client()
        tools = create_atulya_tools(bank_id="test", client=client)
        assert len(tools) == 3

    def test_include_retain_only(self):
        client = _mock_client()
        tools = create_atulya_tools(
            bank_id="test",
            client=client,
            include_retain=True,
        ...
        assert len(tools) == 1
        assert tools[0].name == "atulya_retain"
    ...
    def test_include_recall_only(self):
        ...
        assert len(tools) == 1
        assert tools[0].name == "atulya_recall"
    ...
    def test_include_reflect_only(self):
        ...
        assert len(tools) == 1
        assert tools[0].name == "atulya_reflect"
    ...
    def test_raises_without_client_or_config(self):
        with pytest.raises(AtulyaError, match="No Atulya API URL"):
            create_atulya_tools(bank_id="test")

    def test_falls_back_to_global_config(self):
        configure(atulya_api_url="http://localhost:8888")
        with patch("atulya_pydantic_ai.tools.Atulya") as mock_cls:
            mock_cls.return_value = _mock_client()
            tools = create_atulya_tools(bank_id="test")
            assert len(tools) == 3
    ...
    def test_explicit_url_overrides_config(self):
        configure(atulya_api_url="http://config:8888")
        with patch("atulya_pydantic_ai.tools.Atulya") as mock_cls:
            mock_cls.return_value = _mock_client()
            create_atulya_tools(
                bank_id="test", atulya_api_url="http://explicit:9999"
            )
            mock_cls.assert_called_once_with(
...
class TestRetainTool:
    ...
    @pytest.mark.asyncio
    async def test_retain_raises_atulya_error(self):
        client = _mock_client()
        ...
        with pytest.raises(AtulyaError, match="Retain failed"):
...
class TestRecallTool:
    ...
    @pytest.mark.asyncio
    async def test_recall_raises_atulya_error(self):
        client = _mock_client()
        ...
        with pytest.raises(AtulyaError, match="Recall failed"):
            await tool_fn("query")
...
class TestReflectTool:
    ...
    @pytest.mark.asyncio
    async def test_reflect_raises_atulya_error(self):
        client = _mock_client()
        ...
        with pytest.raises(AtulyaError, match="Reflect failed"):
            await tool_fn("query")
