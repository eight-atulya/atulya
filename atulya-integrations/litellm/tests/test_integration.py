"""Integration tests for atulya-litellm."""

import pytest

from atulya_litellm import (
    configure,
    set_defaults,
    get_defaults,
    enable,
...
from atulya_litellm.callbacks import AtulyaCallback
...
class TestConfiguration:
    ...
    def test_configure_with_all_options(self):
        ...
        assert config.atulya_api_url == "http://custom:9999"
...
class TestCallback:
    """Tests for the AtulyaCallback class."""
    ...
    def test_format_memories(self):
        ...
        configure(atulya_api_url="http://localhost:8888", verbose=False)
...
class TestContextManager:
    """Tests for the atulya_memory context manager."""
    ...
    def test_context_manager_enables_and_disables(self):
        ...
        with atulya_memory(bank_id="test-agent"):
            assert is_enabled() is True
            defaults = get_defaults()
            assert defaults.bank_id == "test-agent"
    ...
    def test_context_manager_restores_previous_config(self):
        ...
        with atulya_memory(bank_id="temporary-agent"):
    ...
    def test_context_manager_with_fact_types(self):
        ...
        with atulya_memory(bank_id="test-agent", fact_types=["world", "opinion"]):
            defaults = get_defaults()
            assert defaults.fact_types == ["world", "opinion"]
...
class TestStreamingSupport:
    ...
    def test_wrap_openai_with_stream_no_error(self):
        ...
        from atulya_litellm.wrappers import wrap_openai
    ...
    def test_wrap_anthropic_with_stream_no_error(self):
        ...
        from unittest.mock import Mock, MagicMock
        from atulya_litellm.wrappers import wrap_anthropic
    ...
    def test_wrap_openai_stream_stores_conversation(self):
        ...
        # Mock the atulya client
        mock_atulya_client = MagicMock()
        with patch.object(wrapped, "_get_atulya_client", return_value=mock_atulya_client):
            # Call with stream=True
            result = wrapped.chat.completions.create(
                model="gpt-4o-mini",
                messages=[{"role": "user", "content": "Hello"}],
            ...
            mock_atulya_client.retain.assert_called_once()
            call_kwargs = mock_atulya_client.retain.call_args[1]
