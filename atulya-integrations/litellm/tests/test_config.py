"""Unit tests for atulya_litellm configuration and defaults."""
...
from atulya_litellm import configure, wrap_openai, wrap_anthropic
from atulya_litellm.config import (
...
from atulya_litellm.wrappers import (
    AtulyaOpenAI,
    AtulyaAnthropic,
    AtulyaCallSettings,
    _merge_settings,
...
class TestConfigure:
    ...
    def test_configure_reads_api_key_from_env(self):
        ...
        with patch.dict(os.environ, {ATULYA_API_KEY_ENV: "test-api-key-123"}):
            config = configure()
    ...
    def test_configure_explicit_values_override_defaults(self):
        ...
        config = configure(
            atulya_api_url="http://custom-url:8888",
            bank_id="custom-bank",
            api_key="custom-key",
        )

        assert config.atulya_api_url == "http://custom-url:8888"
...
class TestWrapOpenAI:
    ...
    def test_wrap_openai_with_only_client(self):
        ...
        assert isinstance(wrapped, AtulyaOpenAI)
        assert wrapped._default_settings.bank_id == DEFAULT_BANK_ID
        assert wrapped._api_url == DEFAULT_ATULYA_API_URL
    ...
    def test_wrap_openai_reads_api_key_from_env(self):
        ...
        with patch.dict(os.environ, {ATULYA_API_KEY_ENV: "env-api-key"}):
            wrapped = wrap_openai(mock_client)
    ...
    def test_wrap_openai_explicit_overrides_defaults(self):
        ...
        wrapped = wrap_openai(
            mock_client,
            bank_id="my-bank",
            atulya_api_url="http://localhost:9999",
            api_key="my-key",
        )
    ...
    def test_wrap_openai_all_settings_kwargs(self):
        """Test wrap_openai() accepts all AtulyaCallSettings fields as kwargs."""
        mock_client = MagicMock()
...
class TestWrapAnthropic:
    ...
    def test_wrap_anthropic_with_only_client(self):
        ...
        assert isinstance(wrapped, AtulyaAnthropic)
        assert wrapped._default_settings.bank_id == DEFAULT_BANK_ID
        assert wrapped._api_url == DEFAULT_ATULYA_API_URL
        assert wrapped._api_key == "test-key"
    ...
    def test_wrap_anthropic_all_settings_kwargs(self):
        """Test wrap_anthropic() accepts all AtulyaCallSettings fields as kwargs."""
        mock_client = MagicMock()
...
class TestMergeSettings:
    ...
    def test_merge_settings_no_overrides(self):
        ...
        defaults = AtulyaCallSettings(bank_id="default-bank", budget="mid")
        kwargs = {"model": "gpt-4", "messages": []}  # No atulya_* kwargs
    ...
    def test_merge_settings_with_overrides(self):
        """Test _merge_settings() applies atulya_* overrides."""
        defaults = AtulyaCallSettings(bank_id="default-bank", budget="mid")
        kwargs = {
            "model": "gpt-4",
            "atulya_bank_id": "override-bank",
            "atulya_budget": "high",
        }
    ...
    def test_merge_settings_ignores_invalid_fields(self):
        """Test _merge_settings() ignores atulya_* kwargs for non-existent fields."""
        defaults = AtulyaCallSettings(bank_id="default-bank")
        kwargs = {
            "atulya_bank_id": "valid-override",
            "atulya_nonexistent_field": "should-be-ignored",
        }
    ...
    def test_merge_settings_all_fields(self):
        """Test _merge_settings() works with all AtulyaCallSettings fields."""
        ...
        override_values = {
            "atulya_bank_id": "override-bank",
            "atulya_document_id": "override-doc",
            "atulya_session_id": "override-session",
            "atulya_store_conversations": False,
            "atulya_inject_memories": False,
            "atulya_budget": "high",
            "atulya_fact_types": ["world", "experience"],
            "atulya_max_memories": 10,
            "atulya_max_memory_tokens": 2048,
            "atulya_include_entities": False,
            "atulya_trace": True,
            "atulya_tags": ["user:bob", "org:acme"],
            "atulya_recall_tags": ["user:bob"],
            "atulya_recall_tags_match": "all",
            "atulya_use_reflect": True,
            "atulya_reflect_context": "test context",
            "atulya_reflect_response_schema": {"type": "object"},
            "atulya_reflect_include_facts": True,
            "atulya_query": "test query",
            "atulya_verbose": True,
        }
...
class TestAtulyaCallSettingsConsistency:
    """Test that AtulyaCallSettings works consistently across wrappers."""

    def test_same_settings_for_openai_and_anthropic(self):
        """Test that OpenAI and Anthropic wrappers use the same AtulyaCallSettings."""
    ...
    def test_new_field_works_for_both_wrappers(self):
        """Test that all AtulyaCallSettings fields work for both wrappers."""
        mock_openai_client = MagicMock()
        mock_anthropic_client = MagicMock()

        # Get all fields from the dataclass
        all_field_names = [f.name for f in fields(AtulyaCallSettings)]
