...
- **Simple Integration** - Just configure, set defaults, enable, and use `atulya_litellm.completion()`
- **Automatic Memory Injection** - Relevant memories are injected into prompts before LLM calls
- **Automatic Conversation Storage** - Conversations are stored to Atulya for future recall (async by default for performance)
...
# Step 4: Use with explicit atulya_query (required when inject_memories=True)
response = atulya_litellm.completion(
    model="gpt-4o-mini",
    messages=[{"role": "user", "content": "What did we discuss about AI?"}],
    atulya_query="What do I know about AI discussions?",  # Required!
)
```

**Important:** When `inject_memories=True` (default), you must provide `atulya_query` to specify what to search for in memory. This ensures intentional, focused memory queries.
...
│  ───────────────────────────────────────────────────────────────────────── │
│  # atulya_litellm queries Atulya for relevant memories                │
│                                                                             │
│  # If use_reflect=False (default) - raw memories:                           │
│  memories = atulya.recall(query="Help me with my Python project")        │
│  # Returns: ["User prefers pytest", "User is building a FastAPI app", ...]  │
│                                                                             │
│  # If use_reflect=True - synthesized context:                               │
│  context = atulya.reflect(query="Help me with my Python project")        │
...
│  ───────────────────────────────────────────────────────────────────────── │
│  # The conversation is stored to Atulya for future recall                │
│  atulya.retain(                                                          │
│      content="User: Help me with my Python project\n"                       │
│              "Assistant: Since you're working on FastAPI..."                │
│  )                                                                          │
│  # Atulya extracts facts: "User asked about Python project help"         │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
atulya_litellm.configure(
    ...
    api_key="your-api-key",        # API key for Atulya authentication
...
Default values for per-call settings. These can be overridden on individual calls using `atulya_*` kwargs:
...
Override any default on individual calls using `atulya_*` kwargs:

```python
response = atulya_litellm.completion(
    model="gpt-4o-mini",
    messages=[...],
    atulya_query="Where is Alice located?",      # REQUIRED when inject_memories=True
    atulya_reflect_context="Currently on floor 3",  # Per-call reflect context override
    # atulya_bank_id="other-bank",               # Override bank_id for this call
)
```
...
```python
atulya_litellm.set_bank_mission(
...
atulya_litellm.set_defaults(bank_id="my-agent", use_reflect=True)
...
# OpenAI
atulya_litellm.completion(model="gpt-4o", messages=messages, atulya_query="greeting")

# Anthropic
atulya_litellm.completion(model="claude-3-5-sonnet-20241022", messages=messages, atulya_query="greeting")

# Groq
atulya_litellm.completion(model="groq/llama-3.1-70b-versatile", messages=messages, atulya_query="greeting")

# Azure OpenAI
atulya_litellm.completion(model="azure/gpt-4", messages=messages, atulya_query="greeting")

# AWS Bedrock
atulya_litellm.completion(model="bedrock/anthropic.claude-3", messages=messages, atulya_query="greeting")

# Google Vertex AI
atulya_litellm.completion(model="vertex_ai/gemini-pro", messages=messages, atulya_query="greeting")
```
...
```python
from atulya_litellm import configure, set_defaults, reflect
...
```python
from atulya_litellm import configure, set_defaults, retain, get_pending_retain_errors
...
```python
from atulya_litellm import configure, set_defaults, enable, completion, get_last_injection_debug
...
response = completion(
    ...
    atulya_query="What is the user's favorite color?",
)
...
|----------|-------------|
| `configure(...)` | Configure static Atulya settings (API URL, auth, storage options) |
...
| `is_configured()` | Check if Atulya is configured with a bank_id |
