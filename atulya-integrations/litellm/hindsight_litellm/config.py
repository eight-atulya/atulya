"""Global configuration for Atulya-LiteLLM integration.

This module provides a clean API for configuring Atulya integration:
...
3. Per-call kwargs (atulya_* prefix) - Override any setting per-call
   - atulya_bank_id, atulya_budget, atulya_inject_memories, etc.
...
# Default Atulya API URL (production)
DEFAULT_ATULYA_API_URL = "https://api.atulya.eightengine.com"
DEFAULT_BANK_ID = "default"
ATULYA_API_KEY_ENV = "ATULYA_API_KEY"
...
@dataclass
class AtulyaCallSettings:
    """Unified settings for Atulya memory operations.

    All fields here can be:
    - Set as defaults via configure() or set_defaults()
    - Overridden per-call via atulya_* kwargs (e.g., atulya_bank_id="other")
    ...
    session_id: Optional[str] = None  # Primary - maps to Atulya's document_id
    ...
    def effective_document_id(self) -> Optional[str]:
        """Get the effective document_id for Atulya API calls.

        Returns session_id if set, otherwise falls back to document_id.
        This maps to Atulya's document_id parameter for retain operations.
        """
        return self.session_id if self.session_id is not None else self.document_id


def _merge_call_settings(
    defaults: AtulyaCallSettings, kwargs: Dict[str, Any]
) -> AtulyaCallSettings:
    """Merge per-call kwargs (atulya_*) with defaults.

    This automatically handles all fields in AtulyaCallSettings.
    When a new field is added to the dataclass, it works here automatically.

    Args:
        defaults: The default settings
        kwargs: The kwargs passed to the call, may contain atulya_* overrides
    ...
    # Override with atulya_* kwargs
    for key, value in kwargs.items():
        if key.startswith("atulya_"):
            setting_name = key[len("atulya_") :]
            if setting_name in valid_fields:
                merged[setting_name] = value

    return AtulyaCallSettings(**merged)


# Backward compatibility alias
AtulyaDefaults = AtulyaCallSettings


@dataclass
class AtulyaConfig:
    """Connection-level configuration for Atulya integration.
    ...
    Attributes:
        ...
        api_key: API key for Atulya authentication
    ...
    atulya_api_url: str = DEFAULT_ATULYA_API_URL
    api_key: Optional[str] = None
    excluded_models: List[str] = field(default_factory=list)
    sync_storage: bool = False
    default_settings: AtulyaCallSettings = field(
        default_factory=AtulyaCallSettings
    )
...
# Global instances
_global_config: Optional[AtulyaConfig] = None
...
def configure(
    ...
    # Per-call defaults (all AtulyaCallSettings fields)
    bank_id: Optional[str] = None,
    ...
    """Configure Atulya integration settings.

    Sets up connection settings and default values for per-call settings.
    All per-call settings can be overridden using atulya_* kwargs.
    ...
    Args:
        ...
        # Per-call defaults (can be overridden with atulya_* kwargs):
    ...
    Returns:
        The configured AtulyaConfig instance

    Example:
        >>> from atulya_litellm import configure, enable
        >>>
        >>> # Minimal usage - just set ATULYA_API_KEY env var
        >>> configure()
        >>> enable()
        >>>
        ...
        ...     model="gpt-4",
        ...     messages=[...],
        ...     atulya_bank_id="other-user",  # Override default
        ... )
    """
    ...
    default_settings = AtulyaCallSettings(
...
def set_defaults(
    ...
    atulya_* kwargs (e.g., atulya_bank_id="other-bank").
    ...
    Returns:
        The updated AtulyaCallSettings instance

    Example:
        >>> from atulya_litellm import configure, set_defaults
        >>> configure()
    ...
    updated_settings = AtulyaCallSettings(
    ...
    _global_config = AtulyaConfig(
        atulya_api_url=_global_config.atulya_api_url,
...
def _create_or_update_bank(
    ...
    try:
        ...
        client = Atulya(base_url=atulya_api_url, api_key=api_key)
        client.create_bank(
            bank_id=bank_id,
            name=name,
            mission=mission,
        ...
        if verbose:
            ...
            logging.getLogger("atulya_litellm").info(
        ...
        if verbose:
            ...
            logging.getLogger("atulya_litellm").warning(
                "atulya_client not installed. Cannot create bank. "
                "Install with: pip install atulya-client"
            )
...
def get_config() -> Optional[AtulyaConfig]:
    """Get the current global configuration.

    Returns:
        The current AtulyaConfig instance, or None if not configured
    """
    return _global_config


def get_defaults() -> Optional[AtulyaCallSettings]:
    """Get the current global defaults for per-call settings.

    Returns:
        The current AtulyaCallSettings instance, or None if not configured
    """
...
def is_configured() -> bool:
    """Check if Atulya has been configured with a valid bank_id.
