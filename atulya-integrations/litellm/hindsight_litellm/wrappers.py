"""Native client wrappers for Atulya memory integration.

This module provides wrappers for native LLM client SDKs (OpenAI, Anthropic)
that automatically integrate with Atulya for memory injection and storage.
...
def _get_client(api_url: str, api_key: Optional[str] = None):
    """Create a fresh Atulya client for the given URL.

    Note: We create a fresh client each time because the atulya_client
    ...
    return Atulya(base_url=api_url, api_key=api_key, timeout=30.0)
...
def recall(
    ...
    atulya_api_url: Optional[str] = None,
) -> RecallResponse:
    """Recall memories from Atulya.
    ...
    Args:
        ...
        atulya_api_url: Override the configured API URL
    ...
    Raises:
        RuntimeError: If Atulya is not configured and no overrides provided
    ...
    api_url = atulya_api_url or (config.atulya_api_url if config else None)
    ...
    if not api_url or not target_bank_id:
        raise RuntimeError(
            "Atulya not configured. Call configure() or provide bank_id and atulya_api_url."
        )
    ...
    try:
        ...
        raise RuntimeError(f"atulya-client not installed: {e}")
...
def _retain_sync(
    ...
    try:
        ...
        debug_info = None
        if verbose:
            logger.info(f"Stored content to Atulya bank: {target_bank_id}")
...
def retain(
    ...
    """Store content to Atulya memory.
...
class _StreamWrapper:
    ...
    def __init__(
        ...
        user_query: str,
        model: str,
        wrapper: "AtulyaOpenAI",
        settings: AtulyaCallSettings,
    ):
        self._stream = stream
...
class AtulyaOpenAI:
    """Wrapper for OpenAI client with Atulya memory integration.
    ...
    Example:
        ...
        ...     atulya_bank_id="other-user",  # Different bank for this call
        ...     atulya_budget="high",          # Higher recall budget
        ...     atulya_inject_memories=False,  # Skip memory injection
        ... )
    """
    ...
    def __init__(
        ...
        atulya_api_url: str,
        api_key: Optional[str] = None,
        default_settings: Optional[AtulyaCallSettings] = None,
        **setting_kwargs,
    ):
        ...
        Args:
            ...
            atulya_api_url: URL of the Atulya API server (required, connection-level)
            api_key: API key for Atulya authentication (connection-level)
            default_settings: Default settings for all calls (alternative to kwargs)
            **setting_kwargs: Default values for any AtulyaCallSettings field
                (e.g., bank_id="my-bank", budget="high", verbose=True)
        """
        self._client = client
        self._api_url = atulya_api_url
        self._api_key = api_key
        self._atulya_client = None

        # Build default settings from kwargs or use provided settings
        if default_settings is not None:
            self._default_settings = default_settings
        else:
            # Filter kwargs to only valid AtulyaCallSettings fields
            valid_fields = {f.name for f in fields(AtulyaCallSettings)}
            settings_kwargs = {
                k: v for k, v in setting_kwargs.items() if k in valid_fields
            }
            self._default_settings = AtulyaCallSettings(**settings_kwargs)

        # Create wrapped chat.completions interface
        self.chat = _WrappedChat(self)

    def _get_atulya_client(self):
        """Get or create the Atulya client."""
        if self._atulya_client is None:
            from atulya_client import Atulya

            self._atulya_client = Atulya(
                base_url=self._api_url,
                api_key=self._api_key,
                timeout=30.0,
            )
        return self._atulya_client

    def _recall_memories(self, query: str, settings: AtulyaCallSettings) -> str:
        ...
        try:
            client = self._get_atulya_client()
    ...
    def _reflect_memories(self, query: str, settings: AtulyaCallSettings) -> str:
    ...
    def _store_conversation(
        ...
        """Store the conversation to Atulya.
        ...
        try:
            ...
            if settings.verbose:
                logger.info(
                    f"Stored conversation to Atulya bank: {settings.bank_id}"
                )
...
class _WrappedChat:
    ...
    def __init__(self, wrapper: AtulyaOpenAI):
        self._wrapper = wrapper
...
class _WrappedCompletions:
    ...
    def create(self, **kwargs) -> Any:
        ...
        Supports per-call overrides via atulya_* kwargs:
        - atulya_bank_id: Override memory bank
        - atulya_budget: Override recall budget (low/mid/high)
        - atulya_inject_memories: Override whether to inject memories
        - atulya_store_conversations: Override whether to store
        - atulya_query: Custom query for memory recall
        - atulya_use_reflect: Use reflect API instead of recall
        - And all other AtulyaCallSettings fields...
        """
        # Merge default settings with per-call overrides
        settings = _merge_settings(self._wrapper._default_settings, kwargs)

        # Remove atulya_* kwargs before passing to OpenAI
        openai_kwargs = {
            k: v for k, v in kwargs.items() if not k.startswith("atulya_")
        }
...
class AtulyaAnthropic:
    """Wrapper for Anthropic client with Atulya memory integration.
    ...
    Example:
        ...
        ...     max_tokens=1024,
        ...     messages=[{"role": "user", "content": "Hello"}],
        ...     atulya_bank_id="other-user",  # Different bank
        ...     atulya_budget="high",          # More memories
        ... )
    """
...
class _WrappedAnthropicMessages:
    ...
    def __init__(self, wrapper: AtulyaAnthropic):
        self._wrapper = wrapper
    ...
    def create(self, **kwargs) -> Any:
        ...
        # Remove atulya_* kwargs before passing to Anthropic
        anthropic_kwargs = {
            k: v for k, v in kwargs.items() if not k.startswith("atulya_")
        }
...
def wrap_openai(
    ...
    **settings_kwargs,
    ...
    """Wrap an OpenAI client with Atulya memory integration.
    ...
    All settings can be overridden per-call using atulya_* kwargs.

    Args:
        client: The OpenAI client instance to wrap
        atulya_api_url: URL of the Atulya API server
            (default: https://api.atulya.eightengine.com)
        api_key: API key for Atulya authentication. If not provided,
            reads from ATULYA_API_KEY environment variable.
        mission: Instructions guiding what Atulya should learn and remember
            (used for mental model generation). If provided, creates/updates the bank.
        bank_name: Optional display name for the bank.
        **settings_kwargs: Default values for any AtulyaCallSettings field:
    ...
    resolved_api_url = atulya_api_url or DEFAULT_ATULYA_API_URL
    resolved_api_key = api_key or os.environ.get(ATULYA_API_KEY_ENV)
    ...
    if mission or bank_name:
        try:
            ...
            hs_client = Atulya(base_url=resolved_api_url, api_key=resolved_api_key)
    ...
    return AtulyaOpenAI(
        client=client,
        atulya_api_url=resolved_api_url,
...
def wrap_anthropic(
    ...
    **settings_kwargs,
    ...
    """Wrap an Anthropic client with Atulya memory integration.
