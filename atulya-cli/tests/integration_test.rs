...
fn test_cli_help() {
    ...
    assert!(stdout.contains("Atulya CLI"));
}
...
fn test_ui_command_without_config() {
    ...
    let temp_dir = std::env::temp_dir().join(format!("atulya-test-ui-{}", std::process::id()));
    std::fs::create_dir_all(&temp_dir).expect("Failed to create temp dir");

    let output = Command::new("cargo")
        .args(["run", "--", "ui"])
        .env_remove("ATULYA_API_URL")
        .env_remove("ATULYA_API_KEY")
        .env("HOME", &temp_dir)
        .output()
        .expect("Failed to execute command");
    ...
    assert!(
        ...
        || stdout.contains("Launching Atulya Control Plane UI")
        || stderr.contains("Configuration error")
        || stderr.contains("ATULYA_API_URL"),
...
fn test_ui_command_with_config() {
    ...
    // 1. Build the control plane: cd atulya-control-plane && npm run build
    // 2. Run: cargo test test_ui_command_with_config -- --ignored

    // Just verify that the ui command accepts the configuration
    let temp_dir = std::env::temp_dir().join(format!("atulya-test-ui-valid-{}", std::process::id()));
    std::fs::create_dir_all(&temp_dir).expect("Failed to create temp dir");

    // Write a minimal config
    let config_dir = temp_dir.join(".config").join("atulya");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Atulya CLI") || output.status.success());
...
fn test_configure_command() {
    ...
    let temp_dir = std::env::temp_dir().join(format!("atulya-test-{}", std::process::id()));
    std::fs::create_dir_all(&temp_dir).expect("Failed to create temp dir");
