...
fn render_header(f: &mut Frame, app: &App, area: Rect) {
    ...
    let title = format!("Atulya Explorer - {}{}", app.view.title(), bank_info);
...
fn render_help(f: &mut Frame, area: Rect) {
    let help_text = vec![
        Line::from(Span::styled("Atulya Explorer - Keyboard Shortcuts", Style::default().fg(BRAND_START).add_modifier(Modifier::BOLD))),
        Line::from(""),
        Line::from(vec![
        ...
        Line::from("  m/e/d       - Switch to Memories / Entities / Documents"),
                ...
                match app.input_mode {
                    InputMode::Normal => {
                        match key.code {
                            ...
                            // Query view controls + view switching
                            KeyCode::Char('m') => {
                                if matches!(app.view, View::Query(_)) {
                                    app.toggle_query_mode();
                                } else if let Some(bank_id) = app.selected_bank_id.clone() {
                                    app.switch_to_view(View::Memories(bank_id))?;
                                }
                            }
                            KeyCode::Char('e') => {
                                if !matches!(app.view, View::Query(_)) {
