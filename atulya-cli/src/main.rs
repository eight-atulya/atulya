...
#[derive(Parser)]
#[command(name = "atulya")]
#[command(about = "Atulya CLI - A living algorithm for machine intelligence (MI)", long_about = None)]
#[command(version)]
...
fn get_after_help() -> String {
    ...
    format!(
        "Current API URL: {} (from {})\n\nRun 'atulya configure' to change the API URL.",
        api_url, source
    )
}

fn get_before_help() -> String {
    ui::get_logo()
}
...
enum Commands {
    ...
    /// Manage Atulya brain runtime and .brain lifecycle
    #[command(subcommand)]
    Brain(BrainCommands),
    ...
    #[command(after_help = "Configuration priority:\n  1. Environment variables (ATULYA_API_URL, ATULYA_API_KEY) - highest priority\n  2. Config file (~/.atulya/config)\n  3. Default (http://localhost:8888)")]
    Configure {
...
enum BrainCommands {
    ...
    /// Manage sub_routine jobs and outputs
    #[command(subcommand)]
    SubRoutine(BrainSubRoutineCommands),
    ...
    Export {
        ...
        /// Output file path (.brain)
        #[arg(long)]
        out: Option<PathBuf>,
    },

    /// Validate a .brain file before import
    ImportValidate {
        /// Bank ID
        bank_id: String,
    ...
    /// Learn from a remote Atulya brain endpoint
    Learn {
        /// Local bank ID
        bank_id: String,

        /// Remote Atulya API endpoint (e.g. http://host:8888)
        remote_endpoint: String,

        /// Remote bank ID to learn from
        remote_bank_id: String,

        /// Optional remote API key
        #[arg(long)]
        remote_api_key: Option<String>,

        /// Learning strategy
        #[arg(long, default_value = "auto", value_parser = ["auto", "distilled", "structured", "raw_mirror"])]
        learning_type: String,

        /// Copy mode
        #[arg(long, default_value = "incremental", value_parser = ["incremental", "full_copy"])]
        mode: String,
    ...
    /// Local system capability scan (CPU/GPU/RAM/disk/network)
    #[command(subcommand)]
    Capabilities(BrainCapabilityCommands),
}

#[derive(Subcommand)]
enum BrainSubRoutineCommands {
    /// Trigger sub_routine asynchronously
    Trigger {
        /// Bank ID
        bank_id: String,

        /// Sub-routine mode
        #[arg(long, default_value = "incremental", value_parser = ["warmup", "incremental", "full_copy"])]
        mode: String,
    ...
    /// Get 24-hour activity histogram
    Histogram {
        /// Bank ID
        bank_id: String,
    },
...
#[derive(Subcommand)]
enum BrainCapabilityCommands {
    /// Capture local machine capabilities and write to a git-trackable JSON file
    Scan {
        /// Optional output file path (.json). Default: atulya-cortex/capabilities/<host>-<timestamp>.json
        #[arg(long)]
        out: Option<PathBuf>,
    },
}
...
fn run() -> Result<()> {
    ...
    let result: Result<()> = match cli.command {
        ...
        // Brain commands
        Commands::Brain(brain_cmd) => match brain_cmd {
            BrainCommands::Status { bank_id } => {
                commands::brain::status(&client, &bank_id, verbose, output_format)
            }
            BrainCommands::SubRoutine(sub_cmd) => match sub_cmd {
                BrainSubRoutineCommands::Trigger {
                    bank_id,
                    mode,
                    horizon_hours,
                    force_rebuild,
                    wait,
                    poll_interval,
                } => commands::brain::subroutine_trigger(
                    &client,
                    &bank_id,
                    &mode,
                    horizon_hours,
                ...
                BrainSubRoutineCommands::Predictions { bank_id, horizon_hours } => {
                    commands::brain::subroutine_predictions(&client, &bank_id, horizon_hours, verbose, output_format)
                }
                BrainSubRoutineCommands::Histogram { bank_id } => {
                    commands::brain::subroutine_histogram(&client, &bank_id, verbose, output_format)
                }
            },
            BrainCommands::Export { bank_id, out } => {
                commands::brain::export(&client, &bank_id, out, verbose, output_format)
            }
            BrainCommands::ImportValidate { bank_id, file } => {
                commands::brain::import_validate(&client, &bank_id, &file, verbose, output_format)
            }
            BrainCommands::Import { bank_id, file } => {
                commands::brain::import(&client, &bank_id, &file, verbose, output_format)
            }
            BrainCommands::Learn {
                bank_id,
                remote_endpoint,
                ...
                wait,
                poll_interval,
            } => commands::brain::learn(
                &client,
                &bank_id,
                &remote_endpoint,
                &remote_bank_id,
                remote_api_key.as_deref().unwrap_or(""),
                &learning_type,
                &mode,
                horizon_hours,
                wait,
            ...
            BrainCommands::Capabilities(cap_cmd) => match cap_cmd {
                BrainCapabilityCommands::Scan { out } => {
                    commands::brain::capabilities_scan(out, output_format)
                }
            },
        },
...
fn handle_configure(api_url: Option<String>, api_key: Option<String>, output_format: OutputFormat) -> Result<()> {
    ...
    if output_format == OutputFormat::Pretty {
        ui::print_info("Atulya CLI Configuration");
        println!();
    ...
    if output_format == OutputFormat::Pretty {
        ...
        println!();
        println!("Note: Environment variables ATULYA_API_URL and ATULYA_API_KEY will override these settings.");
    } else {
...
fn handle_ui(output_format: OutputFormat) -> Result<()> {
    ...
    if output_format == OutputFormat::Pretty {
        ui::print_info("Launching Atulya Control Plane UI...");
        println!();
        println!("  API URL: {}", api_url);
        println!();
    }

    // Run npx @eight-atulya/atulya-control-plane --api-url {api_url}
    let status = Command::new("npx")
        .arg("@eight-atulya/atulya-control-plane")
        .arg("--api-url")
        .arg(api_url)
        .status();
    ...
    match status {
        ...
        Err(e) => {
            ...
            ui::print_info("You can also install the control plane globally: npm install -g @eight-atulya/atulya-control-plane");
            std::process::exit(1);
        }
    }
