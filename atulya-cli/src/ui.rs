...
use colored::*;
use atulya_client::types::ChunkData;
use indicatif::{ProgressBar, ProgressStyle};
use std::io::{self, Write};

/// Banner logo loaded from a dedicated asset file.
const LOGO: &str = include_str!("logo.ansi");
const BRAND_RED: (u8, u8, u8) = (239, 68, 68); // #ef4444
    ...
    LOGO.lines()
        .map(|line| {
            if line.trim().is_empty() {
                ...
                format!(
                    "\x1b[38;2;{};{};{}m{}\x1b[0m",
                    BRAND_RED.0, BRAND_RED.1, BRAND_RED.2, line
                )
            }
        })
        .collect::<Vec<_>>()
