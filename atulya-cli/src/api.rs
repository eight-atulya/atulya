//! API client wrapper
//!
//! This module provides a thin wrapper around the auto-generated atulya-client
//! to bridge from the CLI's synchronous code to the async API client.

use anyhow::Result;
use atulya_client::Client as AsyncClient;
pub use atulya_client::types;
...
#[derive(Debug, Serialize, Deserialize)]
pub struct BrainAsyncOperationResponse {
...
#[derive(Debug, Serialize, Deserialize)]
pub struct BrainRuntimeStatusResponse {
    pub enabled: bool,
    ...
    pub native_library_loaded: bool,
...
#[derive(Debug, Serialize, Deserialize)]
pub struct SubRoutinePredictionResponse {
    pub bank_id: String,
...
#[derive(Debug, Serialize, Deserialize)]
pub struct SubRoutineHistogramResponse {
    pub bank_id: String,
    pub histogram: Vec<PredictionPoint>,
    #[serde(default)]
    pub sample_count: i64,
    pub source_snapshot_id: Option<String>,
    pub model_signature: Option<String>,
...
#[derive(Debug, Serialize, Deserialize)]
pub struct BrainImportValidationResponse {
    pub valid: bool,
...
#[derive(Debug, Serialize, Deserialize)]
pub struct BrainImportResponse {
    pub bank_id: String,
    pub file_path: String,
    pub size_bytes: i64,
    pub format_version: Option<i64>,
...
impl ApiClient {
    ...
    pub fn trigger_sub_routine(
        ...
        force_rebuild: bool,
        _verbose: bool,
    ) -> Result<BrainAsyncOperationResponse> {
        self.runtime.block_on(async {
            let url = format!("{}/v1/default/banks/{}/sub-routine", self.base_url, bank_id);
            ...
            if !response.status().is_success() {
                let status = response.status();
                let text = response.text().await.unwrap_or_default();
                anyhow::bail!("Sub-routine trigger failed ({}): {}", status, text);
            }
            Ok(response.json::<BrainAsyncOperationResponse>().await?)
        })
    }

    pub fn get_brain_status(&self, bank_id: &str, _verbose: bool) -> Result<BrainRuntimeStatusResponse> {
        self.runtime.block_on(async {
            let url = format!("{}/v1/default/banks/{}/brain/status", self.base_url, bank_id);
            let response = self.http_client.get(&url).send().await?;
            if !response.status().is_success() {
                let status = response.status();
                let text = response.text().await.unwrap_or_default();
                anyhow::bail!("Brain status failed ({}): {}", status, text);
            }
            Ok(response.json::<BrainRuntimeStatusResponse>().await?)
        })
    }

    pub fn get_sub_routine_predictions(
        &self,
        bank_id: &str,
        horizon_hours: i64,
        _verbose: bool,
    ) -> Result<SubRoutinePredictionResponse> {
        self.runtime.block_on(async {
            let url = format!(
                "{}/v1/default/banks/{}/sub-routine/predictions?horizon_hours={}",
                self.base_url, bank_id, horizon_hours
            );
            let response = self.http_client.get(&url).send().await?;
            if !response.status().is_success() {
                let status = response.status();
                let text = response.text().await.unwrap_or_default();
                anyhow::bail!("Sub-routine predictions failed ({}): {}", status, text);
            }
            Ok(response.json::<SubRoutinePredictionResponse>().await?)
        })
    }

    pub fn get_sub_routine_histogram(&self, bank_id: &str, _verbose: bool) -> Result<SubRoutineHistogramResponse> {
        self.runtime.block_on(async {
            let url = format!("{}/v1/default/banks/{}/sub-routine/histogram", self.base_url, bank_id);
            let response = self.http_client.get(&url).send().await?;
            if !response.status().is_success() {
                let status = response.status();
                let text = response.text().await.unwrap_or_default();
                anyhow::bail!("Sub-routine histogram failed ({}): {}", status, text);
            }
            Ok(response.json::<SubRoutineHistogramResponse>().await?)
        })
    }

    pub fn export_brain_snapshot(&self, bank_id: &str, _verbose: bool) -> Result<Vec<u8>> {
        self.runtime.block_on(async {
            let url = format!("{}/v1/default/banks/{}/brain/export", self.base_url, bank_id);
            let response = self.http_client.get(&url).send().await?;
            if !response.status().is_success() {
                let status = response.status();
                let text = response.text().await.unwrap_or_default();
                anyhow::bail!("Brain export failed ({}): {}", status, text);
            }
            Ok(response.bytes().await?.to_vec())
        })
    }

    pub fn validate_brain_import(
        &self,
        bank_id: &str,
        file_path: &Path,
        _verbose: bool,
    ) -> Result<BrainImportValidationResponse> {
        self.runtime.block_on(async {
            let raw = std::fs::read(file_path)?;
            let filename = file_path
                .file_name()
                .and_then(|s| s.to_str())
                .unwrap_or("snapshot.brain")
                .to_string();
            ...
            let url = format!("{}/v1/default/banks/{}/brain/import/validate", self.base_url, bank_id);
            let response = self.http_client.post(&url).multipart(form).send().await?;
            if !response.status().is_success() {
                let status = response.status();
                let text = response.text().await.unwrap_or_default();
                anyhow::bail!("Brain import validation failed ({}): {}", status, text);
            }
            Ok(response.json::<BrainImportValidationResponse>().await?)
        })
    }

    pub fn import_brain_snapshot(
        &self,
        bank_id: &str,
        file_path: &Path,
        _verbose: bool,
    ) -> Result<BrainImportResponse> {
        self.runtime.block_on(async {
            let raw = std::fs::read(file_path)?;
            let filename = file_path
                .file_name()
            ...
            let url = format!("{}/v1/default/banks/{}/brain/import", self.base_url, bank_id);
            let response = self.http_client.post(&url).multipart(form).send().await?;
            if !response.status().is_success() {
                let status = response.status();
                let text = response.text().await.unwrap_or_default();
                anyhow::bail!("Brain import failed ({}): {}", status, text);
            }
            Ok(response.json::<BrainImportResponse>().await?)
        })
    }
    ...
    pub fn brain_learn(
        ...
        self.runtime.block_on(async {
            let url = format!("{}/v1/default/banks/{}/brain/learn", self.base_url, bank_id);
            let response = self.http_client.post(&url).json(request).send().await?;
            if !response.status().is_success() {
                let status = response.status();
                let text = response.text().await.unwrap_or_default();
                anyhow::bail!("Brain learn failed ({}): {}", status, text);
            }
            Ok(response.json::<BrainAsyncOperationResponse>().await?)
        })
    }
