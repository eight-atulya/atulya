                              ...
                              {m.tags.length > 0 && (
                                <div className="flex gap-1">
                                  {m.tags.slice(0, 2).map((tag) => (
                                    <span
                                      key={tag}
                                      className="px-1.5 py-0.5 rounded text-xs bg-red-500/10 text-red-600 dark:text-red-400"
                                    >
                                      {tag}
                                    </span>
                                  ))}
...
function MentalModelDetailPanel({
  ...
  const getFactTypeDisplay = (fact: any) => {
    ...
    if (fact.factType === "world") {
      return { label: "world", color: "bg-red-500/10 text-red-600 dark:text-red-400" };
    }
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {mentalModel.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-1 rounded text-xs font-medium bg-red-500/10 text-red-600 dark:text-red-400"
                    >
                      {tag}
                    </span>
                  ))}
