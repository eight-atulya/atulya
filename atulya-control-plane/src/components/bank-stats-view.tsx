/**
 * BankStatsView — rich memory store analytics dashboard.
 * Sections: Memory Store (inline stat strip + distribution bars + link types),
 *           Consolidation (progress + mental models), Activity (timeseries chart
 *           + operations card), and atulya-specific brain runtime panel.
 *
 */
...
function formatBucketLabel(iso: string, trunc: string): string {
  ...
  if (trunc === "day") return d.toLocaleDateString(undefined, { month: "short", day: "numeric" });
...
function OperationsCard({ byStatus }: { byStatus: Record<string, number> }) {
  const entries = Object.entries(byStatus).map(([status, value]) => ({
    status,
    value,
    color: OPS_STATUS_COLORS[status] || "#6b7280",
  }));
            ...
            <div className="h-1.5 flex w-full rounded-full overflow-hidden bg-muted">
              {visible.map((e) => (
                <div
                  key={e.status}
                  className="h-full"
                  style={{ width: `${(e.value / total) * 100}%`, backgroundColor: e.color }}
                  title={`${e.status}: ${e.value.toLocaleString()}`}
                />
              ))}
            </div>
            <div className="space-y-1.5">
              {visible.map((e) => (
                <div key={e.status} className="flex items-center gap-2 text-xs">
                  ...
                  />
                  <span className="text-muted-foreground flex-1 capitalize">{e.status}</span>
                  <CompactNumber
                    value={e.value}
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <ProgressRow done={done} total={total} doneColor="#10b981" />
        <div className="grid grid-cols-4 gap-3 pt-1">
          ...
          <div className="space-y-0.5">
            <div className="flex items-center gap-1.5">
              <XCircle
                className={`w-3 h-3 ${hasFailed ? "text-red-600 dark:text-red-400" : "text-muted-foreground/50"}`}
              />
              <span
                className={`text-[10px] uppercase tracking-wider font-medium ${hasFailed ? "text-red-600 dark:text-red-400" : "text-muted-foreground"}`}
              >
                Failed
              </span>
            </div>
            <div className="flex items-center gap-1">
              <span
                className={`text-base font-semibold tabular-nums ${hasFailed ? "text-red-600 dark:text-red-400" : "text-foreground"}`}
              >
              ...
              {hasFailed && onRecover && (
                <button
                  onClick={onRecover}
                  title="Recover failed consolidations"
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {total === 0 ? (
          ...
          <>
            <ProgressRow done={upToDate} total={total} doneColor="#10b981" />
...
const AXIS_TICK_STYLE = { fontSize: 11, fill: CHART_COLORS.mutedFg, fontWeight: 500 };
...
export function BankStatsView() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setMentalModels((mentalModelsData.items || []) as MentalModel[]);
  ...
  const loadTimeseries = async () => {
    ...
    try {
      ...
      // silently ignore — timeseries endpoint may not exist yet on older builds
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── METADATA COUNTERS (directives, mental models summary) ─────────── */}
      <section>
      ...
      {/* ── ATULYA BRAIN RUNTIME (sub_routine feature-flagged) ───────────── */}
      {subRoutineEnabled && (
        <section>
          <SectionHeading>Atulya Brain Runtime</SectionHeading>
          <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <p className="text-xs text-muted-foreground">
                {brainStatus?.exists ? "ready" : "not built"}
              </p>
            </div>
            ...
            {brainPredictions?.predictions && brainPredictions.predictions.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {brainPredictions.predictions.slice(0, 3).map((item) => (
                  ...
                  >
                    {item.hour_utc.toString().padStart(2, "0")}:00 UTC (
