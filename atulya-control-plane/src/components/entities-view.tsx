/**
 * EntitiesView — entity list + constellation co-occurrence graph.
 * Relations view: canvas star-field graph with recency heat + co-occurrence size.
 * List view: paginated table (existing behaviour, preserved intact).
 *
 */
...
import { Constellation, convertEntityGraphData } from "./constellation";
import type { GraphNode } from "../features/graph/components/graph-2d";
...
export function EntitiesView() {
  ...
  const loadEntityDetail = async (entityId: string) => {
    if (!currentBank) return;
    // Guard: backend get_entity requires a UUID. Skip non-UUID ids (e.g. canonical names)
    // that some graph implementations use as node identifiers.
  ...
  const constellationData = useMemo(() => {
    ...
    return convertEntityGraphData(graphData);
  }, [graphData]);

  // Node size — sqrt-scaled by total co-occurrence weight (3px isolated → 14px hub).
  ...
  // Node heat — most recent co-occurrence (cool = stale, warm = fresh).
  ...
  return (
    <div>
      ...
      {/* ── Entity Detail Panel — fixed right overlay ── */}
      {selectedEntity && (
