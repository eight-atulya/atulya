...
import type { GraphData, GraphNode, GraphLink } from "../features/graph/components/graph-2d";
...
interface PreparedNode {
  ...
  heatColor: string;
  linkCount: number;
  /** Normalized 0–1 link degree for size mapping */
  normDegree: number;
  /** Outer accent ring colour for fact_type glyph (null = no ring) */
...
export interface ConstellationProps {
  ...
  /** Return radius in canvas pixels (2–14 typical). If omitted, uses log-scaled link degree. */
  nodeSizeFn?: (node: GraphNode) => number;
  compactLabels?: boolean;
  /** Return 0–1 heat value; drives dot colour via heat gradient. */
  nodeHeatFn?: (node: GraphNode) => number;
  heatLegendLabel?: string;
  heatLegendEndpoints?: [string, string];
  sizeLegendLabel?: string;
  /** Return a hex colour for an outer accent ring (encodes fact_type / category). */
  nodeRingFn?: (node: GraphNode) => string | null;
}
...
export function Constellation({
  ...
  const { preparedNodes, linksByNode, linksWithIndices } = useMemo(() => {
    ...
    const nodes: PreparedNode[] = data.nodes.map((node, i) => {
      ...
      // Normalized log degree — drives size when no external nodeSizeFn
      const normDegree = maxLinkCount > 0 ? Math.log1p(lc) / Math.log1p(maxLinkCount) : 0;
      ...
      return {
        ...
        linkCount: lc,
        normDegree,
        ringColor: nodeRingFn?.(node) ?? null,
      };
    });

    const linksIdx: Array<{ a: number; b: number; color: string; type: string }> = [];
    ...
    return { preparedNodes: nodes, linksByNode: byNode, linksWithIndices: linksIdx };
  }, [data, nodeColorFn, linkColorFn, nodeHeatFn, nodeRingFn]);
  ...
  const animate = useCallback(() => {
    ...
    for (const i of order) {
      ...
      // Size: external fn (already in 2–14 range) → or log-scaled degree (3–13px)
      // NOTE: r is in screen-space pixels and does NOT scale with zoom.
      // Node size encodes a data attribute (recall count / degree); the camera zoom
      // should only move you closer to nodes, not change what they represent.
      const baseR = nodeSizeFn
        ? Math.max(2, Math.min(14, nodeSizeFn(n.node)))
        : 3 + n.normDegree * 10;
      const r = baseR;
      const baseAlpha = 0.5 + n.normDegree * 0.45;

      // ── outer glow for high-degree nodes ──────────────────────────────────
      if (n.normDegree > 0.35 && !isHovered && hoverIndex < 0) {
        ctx.beginPath();
        ctx.arc(sx, sy, r * 2.4, 0, Math.PI * 2);
        ctx.fillStyle = n.heatColor;
        ctx.globalAlpha = 0.04 + n.normDegree * 0.07;
        ctx.fill();
      }
      ...
      // ── outer ring (fact_type glyph) ──────────────────────────────────────
      if (n.ringColor && r >= 2) {
        ctx.beginPath();
        ctx.arc(sx, sy, r + 1.8, 0, Math.PI * 2);
        ctx.strokeStyle = n.ringColor;
        ctx.lineWidth = isHovered ? 1.5 : 1;
        ctx.globalAlpha = isHovered ? 0.9 : isNeighbor ? 0.75 : hoverIndex >= 0 ? 0.07 : 0.45;
        ctx.stroke();
      }
    ...
    ctx.textAlign = "left";
    ctx.fillText(
      `${preparedNodes.length} nodes · ${visibleCount} visible · ${labelsShown} labels · ${linksDrawn} links · zoom ${zoom.toFixed(2)}x`,
      12,
      H - 12
    );
    ...
    if (sizeLegendLabel) {
      ...
      ctx.fillText("low", 12, 98);
      const fewW = ctx.measureText("low").width;
      const dotsStart = 12 + fewW + 8;
      const dotColor = isDark ? "#a1a1aa" : "#52525b";
      ctx.fillStyle = dotColor;
      ctx.beginPath();
      ctx.arc(dotsStart + 3, 94, 2.5, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(dotsStart + 16, 94, 5, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(dotsStart + 34, 94, 8, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = labelColor;
      ctx.fillText("high", dotsStart + 48, 98);
    }
  ...
  return (
    <div
      ...
      style={
        isFullscreen
          ? { position: "fixed", inset: 0, zIndex: 50, background: isDark ? "#09090b" : "#ffffff" }
...
/**
 * Convert entity graph API response to constellation-compatible GraphData.
 * Handles the `{nodes: [{data: ...}], edges: [{data: ...}]}` envelope.
 */
export function convertEntityGraphData(entityGraphData: {
  ...
  const nodes = (entityGraphData.nodes || []).map((n) => ({
    id: n.data.id,
    label: n.data.label || n.data.id.substring(0, 12),
    color: n.data.color,
  }));
  const links = (entityGraphData.edges || []).map((e) => ({
    source: e.data.source,
    target: e.data.target,
    color: e.data.color,
    type: e.data.linkType || (e.data.lineStyle === "dashed" ? "temporal" : "entity"),
    entity: e.data.entityName,
