...
const OPERATION_TYPE_OPTIONS = [
  ...
  { value: "sub_routine", label: "Sub Routine" },
...
export function BankOperationsView() {
  ...
  const loadOperations = useCallback(
    ...
    [currentBank, limit, offset, statusFilter, taskTypeFilter]
  );
  ...
  useEffect(() => {
    if (currentBank) {
      ...
      const interval = setInterval(() => {
        void loadOperations(statusFilter, offset, taskTypeFilter);
      }, 5000);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Select
            ...
            onValueChange={(value) => handleTaskTypeFilterChange(value === "all" ? null : value)}
          >
      ...
      <div>
        {operations.length > 0 ? (
                    ...
                    >
                      ...
                      <TableCell className="font-medium">
                        {op.task_type === "sub_routine" ? (
                          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20">
                            sub_routine
                          </span>
                        ) : (
                          op.task_type
          ...
          <p className="text-muted-foreground text-center py-8 text-sm">
            No {statusFilter ? `${statusFilter} ` : ""}
            {taskTypeFilter ? `${taskTypeFilter} ` : ""}operations
          </p>
        )}
      </div>
