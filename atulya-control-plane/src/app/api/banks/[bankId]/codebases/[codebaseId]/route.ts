...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${DATAPLANE_URL}/v1/default/banks/${bankId}/codebases/${codebaseId}${request.nextUrl.search}`,
      {
        method: "GET",
    ...
    console.error("Error getting codebase:", error);
    return NextResponse.json({ error: "Failed to get codebase" }, { status: 500 });
  }
}
