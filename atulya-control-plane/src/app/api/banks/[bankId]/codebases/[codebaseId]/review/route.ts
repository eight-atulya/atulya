...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${DATAPLANE_URL}/v1/default/banks/${bankId}/codebases/${codebaseId}/review${request.nextUrl.search}`,
      {
        method: "GET",
    ...
    console.error("Error fetching codebase review:", error);
    return NextResponse.json({ error: "Failed to fetch codebase review" }, { status: 500 });
  }
}
