...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${DATAPLANE_URL}/v1/default/banks/${bankId}/codebases/${codebaseId}/files${request.nextUrl.search}`,
      {
        method: "GET",
    ...
    console.error("Error listing codebase files:", error);
    return NextResponse.json({ error: "Failed to list codebase files" }, { status: 500 });
  }
}
