...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${DATAPLANE_URL}/v1/default/banks/${bankId}/codebases/${codebaseId}/chunks${request.nextUrl.search}`,
      {
        method: "GET",
    ...
    console.error("Error listing codebase chunks:", error);
    return NextResponse.json({ error: "Failed to list codebase chunks" }, { status: 500 });
  }
}
