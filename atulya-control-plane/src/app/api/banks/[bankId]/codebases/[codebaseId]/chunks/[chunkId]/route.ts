...
export async function GET(
  ...
  try {
    const { bankId, codebaseId, chunkId } = await params;
    ...
    console.error("Error fetching codebase chunk detail:", error);
    return NextResponse.json({ error: "Failed to fetch codebase chunk detail" }, { status: 500 });
  }
}
