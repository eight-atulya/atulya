...
export async function POST(request: Request, { params }: { params: Promise<{ bankId: string }> }) {
  try {
    ...
    const res = await fetch(`${DATAPLANE_URL}/v1/default/banks/${bankId}/forge/jobs`, {
      method: "POST",
    ...
    console.error("Error submitting forge job:", error);
    return NextResponse.json({ error: "Failed to submit forge job" }, { status: 500 });
  }
}
