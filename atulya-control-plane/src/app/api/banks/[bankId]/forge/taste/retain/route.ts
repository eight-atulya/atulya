...
export async function POST(request: Request, { params }: { params: Promise<{ bankId: string }> }) {
  try {
    ...
    const res = await fetch(`${DATAPLANE_URL}/v1/default/banks/${bankId}/forge/taste/retain`, {
      method: "POST",
    ...
    console.error("Error retaining taste sets:", error);
    return NextResponse.json({ error: "Failed to retain taste sets" }, { status: 500 });
  }
}
