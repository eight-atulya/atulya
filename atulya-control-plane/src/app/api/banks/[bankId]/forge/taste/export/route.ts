...
export async function POST(request: Request, { params }: { params: Promise<{ bankId: string }> }) {
  try {
    ...
    const res = await fetch(`${DATAPLANE_URL}/v1/default/banks/${bankId}/forge/taste/export`, {
      method: "POST",
    ...
    console.error("Error exporting taste dataset:", error);
    return NextResponse.json({ error: "Failed to export taste dataset" }, { status: 500 });
  }
}
