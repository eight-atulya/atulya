...
cd atulya-api && uv run pytest tests/

# Run specific test file
cd atulya-api && uv run pytest tests/test_http_api_integration.py -v

# Run single test function
cd atulya-api && uv run pytest tests/test_retain.py::test_retain_simple -v

# Lint and format
cd atulya-api && uv run ruff check .
cd atulya-api && uv run ruff format .

# Type checking (uses ty - extremely fast type checker from Astral)
cd atulya-api && uv run ty check atulya_api/
```
...
1. **Create a new migration file** in `atulya-api/atulya_api/alembic/versions/`:
...
3. **Run migrations locally**:
   ...
   uv run atulya-admin run-db-migration --schema tenant_xyz
   ```
...
When adding or modifying parameters in the dataplane API (atulya-api), you must also update the control plane routes that proxy to it:

1. **API Routes** (`atulya-control-plane/src/app/api/`):
...
2. **Client types** (`atulya-control-plane/src/lib/api.ts`):
...
1. **config.py** (`atulya-api/atulya_api/config.py`):
   - Add `ENV_*` constant for the environment variable name (e.g., `ENV_MY_SETTING = "ATULYA_API_MY_SETTING"`)
   - Add `DEFAULT_*` constant for the default value
   - Add field to `AtulyaConfig` dataclass with type annotation
...
2. **main.py** (`atulya-api/atulya_api/main.py`):
   - Add field to the manual `AtulyaConfig()` constructor call (search for "CLI override")
...
5. **Documentation** (`atulya-docs/docs/developer/configuration.md`):
...
Required env vars:
- `ATULYA_API_LLM_PROVIDER`: openai, anthropic, gemini, groq, ollama, lmstudio
- `ATULYA_API_LLM_API_KEY`: Your API key
- `ATULYA_API_LLM_MODEL`: Model name (e.g., gpt-4o-mini, claude-sonnet-4-20250514)

Optional (uses local models by default):
- `ATULYA_API_EMBEDDINGS_PROVIDER`: local (default) or tei
- `ATULYA_API_RERANKER_PROVIDER`: local (default) or tei
- `ATULYA_API_DATABASE_URL`: External PostgreSQL (uses embedded pg0 by default)
- `ATULYA_API_ENABLE_BANK_CONFIG_API`: Enable per-bank config API (default: true)
