/**
 * Tests for Atulya TypeScript client.
 *
 * These tests require a running Atulya API server.
 */

import { AtulyaClient } from '../src';
...
beforeAll(() => {
    client = new AtulyaClient({ baseUrl: ATULYA_API_URL });
});
...
describe('TestBankStats', () => {
    ...
    test('get bank stats', async () => {
        ...
        const apiClient = createClient(createConfig({ baseUrl: ATULYA_API_URL }));
