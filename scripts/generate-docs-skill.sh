...
# Generate agent skill from Atulya documentation
# Converts docs/ to skills/atulya-docs/ for AI agent consumption

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(dirname "$SCRIPT_DIR")"
DOCS_DIR="$ROOT_DIR/atulya-docs/docs"
EXAMPLES_DIR="$ROOT_DIR/atulya-docs/examples"
SKILL_DIR="$ROOT_DIR/skills/atulya-docs"
...
print_info "Generating Atulya documentation skill..."
...
- Understand Atulya architecture and core concepts
- Learn about retain/recall/reflect operations
- Configure memory banks and dispositions
- Set up the Atulya API server (Docker, Kubernetes, pip)
...
- Configuration uses `ATULYA_API_*` environment variables
...
**Auto-generated** from `atulya-docs/docs/`. Run `./scripts/generate-docs-skill.sh` to update.
EOF
