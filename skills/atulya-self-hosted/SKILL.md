---
name: atulya-self-hosted
description: Store team knowledge, project conventions, and learnings from tasks. Use to remember what works and recall context before new tasks. Connects to a self-hosted Atulya server. (user)
---

# Atulya Memory Skill (Self-Hosted)

You have persistent memory via a **self-hosted Atulya server**. This memory bank can be **shared with the team**, so knowledge stored here benefits everyone working on this codebase.

**Proactively store team knowledge and recall context** to provide better assistance.

## Setup Check (First-Time Only)

Before using memory commands, verify the Atulya CLI is configured:
...
1. **Install the CLI** (if `atulya` command not found):
   ```bash
   curl -fsSL https://atulya.eightengine.com/get-cli | bash
   ```

2. **Create the config file** - ask the user for:
   - **API URL**: Their self-hosted Atulya server URL (e.g., `https://atulya.mycompany.com`)
   - **API Key**: Their authentication key

   ```bash
   mkdir -p ~/.atulya
   cat > ~/.atulya/config << 'EOF'
   api_url = "<user's server URL>"
   api_key = "<user's API key>"
   EOF
   chmod 600 ~/.atulya/config
   ```
...
```bash
atulya memory retain <bank-id> "Project uses ESLint with Airbnb config and Prettier for formatting"
atulya memory retain <bank-id> "Running tests requires NODE_ENV=test" --context procedures
atulya memory retain <bank-id> "Build failed when using Node 18, works with Node 20" --context learnings
atulya memory retain <bank-id> "Alice prefers verbose commit messages with context" --context preferences
```
...
```bash
atulya memory recall <bank-id> "project conventions and coding standards"
atulya memory recall <bank-id> "Alice preferences for this project"
atulya memory recall <bank-id> "what issues have we encountered before"
atulya memory recall <bank-id> "how does the auth module work"
```
...
```bash
atulya memory reflect <bank-id> "How should I approach this task based on past experience?"
```
