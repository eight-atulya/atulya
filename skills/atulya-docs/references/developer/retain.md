...
Atulya doesn't just store what was said — it captures **why**, **how**, and **what it means**.
...
Atulya automatically identifies and tracks **entities** — the people, organizations, and concepts that matter.
...
Memories aren't isolated — Atulya creates a **knowledge graph** with four types of connections:
...
Set `retain_mission` and `retain_extraction_mode` via the [bank config API](/developer/api/memory-banks#retain-configuration) or the [`ATULYA_API_RETAIN_MISSION`](/developer/configuration#retain) environment variable.

---

## Observation Consolidation

After `retain()` completes, Atulya automatically triggers **observation consolidation** in the background. This process:
