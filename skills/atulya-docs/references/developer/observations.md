...
Set `observations_mission` via the [bank config API](/developer/api/memory-banks#observations-configuration) or the [`ATULYA_API_OBSERVATIONS_MISSION`](/developer/configuration#observations) environment variable.
...
Observations are derived from source memories. When source memories are removed, Atulya automatically keeps observations consistent:
