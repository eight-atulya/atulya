...
> **💡 Prerequisites**
> 
Make sure you've [installed Atulya](../installation) and completed the [Quick Start](./quickstart).
...
```bash
# Store a single fact
atulya retain my-bank "Alice joined Google in March 2024 as a Senior ML Engineer"

# Store from a file
atulya retain my-bank --file conversation.txt --context "Daily standup"

# Store multiple files
atulya retain my-bank --files docs/*.md
```
...
```bash
# Basic search
atulya recall my-bank "What does Alice do at Google?"

# Search with options
atulya recall my-bank "What happened last spring?" \
    --budget high \
    --max-tokens 8192 \
    --fact-type world

# Verbose output (shows weights and sources)
atulya recall my-bank "Tell me about Alice" -v
```
...
```bash
# Basic reflect
atulya reflect my-bank "Should we adopt TypeScript for our backend?"

# Verbose output (shows sources and observations)
atulya reflect my-bank "What are Alice's strengths for the team lead role?" -v

# With higher reasoning budget
atulya reflect my-bank "Analyze our tech stack" --budget high
```
