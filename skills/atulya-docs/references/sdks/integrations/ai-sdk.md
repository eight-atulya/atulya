...
The `@eight-atulya/atulya-ai-sdk` package integrates [Atulya](https://atulya.eightengine.com) memory with the [Vercel AI SDK](https://ai-sdk.dev). It provides five ready-to-use tools for retaining, recalling, and reflecting on long-term memories.

## Installation

```bash
npm install @eight-atulya/atulya-ai-sdk @eight-atulya/atulya-client ai
```

## Setup

Create a Atulya client and pass it to `createAtulyaTools` along with a `bankId`. The `bankId` identifies the memory store for this session—typically a user ID.
...
const client = new AtulyaClient({ baseUrl: process.env.ATULYA_API_URL! });

const agent = new ToolLoopAgent({
  model: openai('gpt-4o'),
  tools: createAtulyaTools({ client, bankId: 'user-123' }),
...
const atulyaClient = new AtulyaClient({
  baseUrl: process.env.ATULYA_API_URL!,
});
